...
class KinLikelihood(object):
    """Class to compute the likelihood associated to binned 2D kinematic
    maps."""
    ...
    def calc_vrms(self, kwargs_lens, kwargs_lens_light, kwargs_special, verbose=False):
        ...
        :param verbose: default False; if True print statements when out
            of bounds return binned vrms [km/s]; if SKiNN not installed
            return nan
        """
    ...
    def logL(self, kwargs_lens, kwargs_lens_light, kwargs_special, verbose=False):
        ...
        :param kwargs_special: cosmology and other kwargs
        :param verbose: default False; if True print statements when out
            of bounds return kinematics log likelihood
        """
        if self.kinematic_NN.SKiNN_installed:
            self.update_image_input(kwargs_lens)
            self.light_map = self.lens_light_model_class.surface_brightness(
    ...
    def convert_to_nn_params(self, kwargs_lens, kwargs_lens_light, kwargs_special):
        """Converts lenstronomy kwargs into input vector for SKiNN, also
        returns whether or not mass and light are aligned.
    ...
    def rescale_distance(self, image, kwargs_special):
        ...
        :param kwargs_special: kwargs with cosmological distances for
            rescaling return rescaled vrms image [km/s]
        """
    ...
    def kwargs_data2image_input(self, kwargs_data):
        ...
        :param kwargs_data: kwargs giving image and describing imaging
            data coordinate transformation
        :return kwargs: coordinate transformation kwargs as input for
            KinNNImageAlign class
        """
    ...
    def update_image_input(self, kwargs_lens):
        """Updates the image_input for rotation with the new values of
        orientation and center of the lens model.

        :param kwargs_lens: lens kwargs with center and PA positions to
            be used as input
        """
    ...
    def auto_binning(self, rotated_map, light_map):
        ...
        :param rotated_map: model vrms map [km/s] in data pixel
            coordinates
        :param light_map: model light map in data pixel coordinates for
            weighting
    ...
    def _logL(self, vrms):
        ...
        :param vrms: binned vrms [km/s] to compare with observed binned
            data
