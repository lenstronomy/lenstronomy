...
import numpy as np
from lenstronomy.LensModel.Util.epl_util import (
    min_approx,
    pol_to_cart,
    cart_to_pol,
    cdot,
...
def _one_dim_lens_eq_calcs(args, phi):
    """Calculates intermediate quantities that are needed for several of the
    ...
    rhat = (
        ...
        / (1 - gamma1**2 - gamma2**2)
        * (
            ((1 + gamma1) * np.cos(phi) + gamma2 * np.sin(phi))
            + 1j * (gamma2 * np.cos(phi) + (1 - gamma1) * np.sin(phi))
        )
    )
    thetahat = (
        1
        / (1 - gamma1**2 - gamma2**2)
        * (
            ((1 + gamma1) * np.sin(phi) - gamma2 * np.cos(phi))
            + 1j * (gamma2 * np.sin(phi) - (1 - gamma1) * np.cos(phi))
        )
    )
    ...
    if abs(t - 1) > 1e-4:
        ...
        Omega_ort = 1j * Omega
        x = ((1 - gamma1) * np.cos(phi) - gamma2 * np.sin(phi)) + 1j * (
...
def _one_dim_lens_eq_both(phi, args):
    ...
    ) * ip**2 + ps(ip, 2 / t) * cdot(Omega, thetahat) ** 2
...
def _getphi(thpl, args):
    """Finds all roots to both versions the 1-dimensional lens equation in phi,
    by doing a grid search for sign changes on the supplied thpl. In the case
    of extrema, refine at the relevant location.

    :param thpl: What points to calculate the equation on use for
    ...
    for i in range(num_phi - 1):
        if y[i + 1] * y[i] <= 0:
            roots.append(
                brentq_inline(_one_dim_lens_eq, thpl[i], thpl[i + 1], args=args)
                % (2 * np.pi)
            )
        elif y_notsmooth[i + 1] * y_notsmooth[i] <= 0:
            roots.append(
                brentq_inline(
                    _one_dim_lens_eq_unsmooth, thpl[i], thpl[i + 1], args=args
                )
                % (2 * np.pi)
            )
    ...
    for i in range(1, num_phi + 1):
        ...
        if (y3 - y2) * (y2 - y1) <= 0 or (y3n - y2n) * (y2n - y1n) <= 0:
            ...
            if ymin * y2 <= 0 and x2 <= xmin <= x3:
                roots.append(
                    brentq_inline(_one_dim_lens_eq, x2, xmin, args=args) % (2 * np.pi)
                )
                roots.append(
                    brentq_inline(_one_dim_lens_eq, xmin, x3, args=args) % (2 * np.pi)
                )
            elif ymin * y2 <= 0 and x1 <= xmin <= x2:
                roots.append(
                    brentq_inline(_one_dim_lens_eq, x1, xmin, args=args) % (2 * np.pi)
                )
                roots.append(
                    brentq_inline(_one_dim_lens_eq, xmin, x2, args=args) % (2 * np.pi)
                )
            elif ymin_ns * y2n <= 0 and x2 <= xmin_ns <= x3:
                roots.append(
                    brentq_inline(_one_dim_lens_eq_unsmooth, x2, xmin_ns, args=args)
                    % (2 * np.pi)
                )
                roots.append(
                    brentq_inline(_one_dim_lens_eq_unsmooth, xmin_ns, x3, args=args)
                    % (2 * np.pi)
                )
            elif ymin_ns * y2n <= 0 and x1 <= xmin_ns <= x2:
                roots.append(
                    brentq_inline(_one_dim_lens_eq_unsmooth, x1, xmin_ns, args=args)
                    % (2 * np.pi)
                )
                roots.append(
                    brentq_inline(_one_dim_lens_eq_unsmooth, xmin_ns, x2, args=args)
                    % (2 * np.pi)
                )
...
def solvelenseq_majoraxis(args, Nmeas=200, Nmeas_extra=50):
    """Solve the lens equation, where the arguments have been properly rotated
    to the major-axis."""
    b, t, y1, y2, q, gamma1, gamma2 = args
    ...
    diff = (
        ...
        + stuff[0]
        + stuff[1] * 1j
        - _alpha_epl_shear(stuff[0], stuff[1], b, q, t, gamma1=gamma1, gamma2=gamma2)
    )
...
def _check_center(kwargs_lens):
    ...
    if len(kwargs_lens) > 1:
        ...
        alpha_x, alpha_y = shear.derivatives(
            kwargs_lens[0]["center_x"], kwargs_lens[0]["center_y"], **kwargs_lens[1]
        )
...
def solve_lenseq_pemd(pos_, kwargs_lens, Nmeas=400, Nmeas_extra=80, **kwargs):
    ...
    gamma *= rotfact**2
    p *= rotfact
    res = solvelenseq_majoraxis(
        (b, t, p.real, p.imag, q, gamma.real, gamma.imag),
        Nmeas=Nmeas,
...
def caustics_epl_shear(
    ...
    """Analytically calculates the caustics of an EPL+shear lens model. Since
    for gamma>2, the outer critical curve does not exist, the option to find
    the curves for a set, finite magnification exists, by supplying maginf, so
    that the routine finds the curve of this magnification, rather than the
    true caustic.

    :param kwargs_lens: List of kwargs in lenstronomy style, following
        ['EPL', 'SHEAR'] format
    :param num_th: resolution.
    :param maginf: the outer critical curve for t>1 will be replaced
        with the curve where the inverse magnification is maginf
    :param sourceplane: if True (default), ray-shoot the calculated
        critical curves to the source plane
    :param return_which: options 'quad' (boundary of area within which
        there are 4 images), 'double' (boundary of area within which
        there are 2 images), 'caustic' (the diamond caustic) and 'cut'
        (the cut, if it exists, that is if t<2, else, if t>2, returns
        the caustic) and None (in that case: return quad, caustic, cut)
    :return: (2,N) array if return_which set, else a tuple of (caustic,
        cut, quad)
    """
    ...
    return (
        M @ (xca_4, yca_4) + cen,
        M @ (xca_cut, yca_cut) + cen,
        M @ pos_tosample + cen,
    )  # Mostly for some backward compatibility
