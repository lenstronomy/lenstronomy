...
class TestKinLikelihood(object):
    def setup(self):
        ...
        center_y = 0.0
        self.kwargs_lens = [
            {'theta_E': 1.5, 'gamma': 2, 'q': mass_q, 'phi': major_axis_pa, 'center_x': center_x, 'center_y': center_y}]
        self.kwargs_lens_light = [
            {'amp': 10, 'R_sersic': 1., 'q': light_q, 'phi': major_axis_pa, 'n_sersic': 3., 'center_x': center_x,
             'center_y': center_y}]
        ...
        image_pixel_grid = PixelGrid(npix, npix, transform_pix2angle, ra_at_xy_0, dec_at_xy_0)
        self.image_data = self.lensLightModel.surface_brightness(image_pixel_grid._x_grid, image_pixel_grid._y_grid,
                                                                 self.kwargs_lens_light)
        ...
        self.kwargs_kin = {'bin_data': binned_dummy_data,
        ...
        self._KinLikelihood = KinLikelihood(_KinData, self.lensModel, self.lensLightModel, self.kwargs_data, idx_lens=0,
                                            idx_lens_light=0)
        if self.kinematic_NN:
            # if NN enabled, calculate realistic values to use as truth for logL tests
            self.truth_vrms = self._KinLikelihood.calc_vrms(self.kwargs_lens, self.kwargs_lens_light,
                                                            self.kwargs_special, verbose=False)
            ...
            _KinBin = KinBin(**self.kwargs_kin)
            _KinData = KinData(_KinBin, self.kinPSF)
            self._KinLikelihood = KinLikelihood(_KinData, self.lensModel, self.lensLightModel, self.kwargs_data,
    ...
    def test_logL(self):
        if self.kinematic_NN:
            ...
            close_logL = self._KinLikelihood.logL(kwargs_lens_close, kwargs_lens_light_close, self.kwargs_special,
                                                  verbose=False)
            far_logL = self._KinLikelihood.logL(kwargs_lens_far, kwargs_lens_light_far, self.kwargs_special,
                                                verbose=False)
            ...
            _KinData = KinData(_KinBin, self.kinPSF)
            var_kin_likelihood = KinLikelihood(_KinData, self.lensModel, self.lensLightModel, self.kwargs_data,
                                               idx_lens=0,
                                               idx_lens_light=0)
    ...
    def test_convert_to_nn_params(self):
        ...
        kwargs_lens_light_test = [{'amp': 10, 'R_sersic': 1., 'q': 1, 'phi': 0, 'n_sersic': 3., 'center_x': 0.,
                                   'center_y': 0.}]
        params, same_orientation = self._KinLikelihood.convert_to_nn_params(kwargs_lens_test, kwargs_lens_light_test,
                                                                            self.kwargs_special)
    ...
    def test_auto_binning(self):
        ...
        _KinBin = KinBin(**self.kwargs_kin)
        _KinData = KinData(_KinBin, kin_psf)
