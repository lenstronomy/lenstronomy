...
class ImageLikelihood(object):
    ...
    def _apply_multiband_offsets(self, kwargs_special):
        """Temporarily apply model-side astrometric corrections (shift and rotation) to
        the coordinate grid.

        The sampled offsets represent corrections applied to the model coordinate system
        during likelihood evaluation. Therefore, when interpreting recovered parameters
        as astrometric offsets between observed bands, the sign is opposite.

        :param kwargs_special: contains 'kwargs_offsets', where each dictionary
            specifies the correction for the corresponding band.
        :return: dict; backup of the original coordinate states to be restored after
            logL evaluation
        """
    ...
    def _restore_multiband_offsets(self, backup_grids):
        ...
        :param backup_grids: dict; coordinate states returned by
            _apply_multiband_offsets
        """
