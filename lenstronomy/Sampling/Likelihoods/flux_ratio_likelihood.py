...
class FluxRatioLikelihood(object):
    ...
    def __init__(
        ...
        """:param lens_model_class: LensModel class instance :param flux_ratios: ratio
        of fluxes of the multiple images (relative to the first appearing in same order
        as the images) :param flux_ratio_errors: errors in the flux ratios (relative to
        the first appearing).

        Alternatively  a log-normal covariance matrix. Note: in the case of a covariance
        matrix, the errors are assumed  to be log-normal, i.e. the logarithms of the
        flux ratios, ln(F[i]/F[0]) are assumed to have a multivariate  Gaussian
        distribution, with the given covariance matrix.
        :param source_type: string, type of source, 'INF' specifies a point source,
                while 'GAUSSIAN' specifies a          finite-size source modeled as a
                Gaussian
    ...
    def logL(self, x_pos, y_pos, kwargs_lens, kwargs_special):
        """:param kwargs_lens: lens model keyword argument list :param kwargs_special:
        dictionary of 'special' keyword parameters :return: log likelihood of the
        measured flux ratios given a model."""
    ...
    def _logL(self, flux_ratios):
        """:param flux_ratios: flux ratios from the model :return: log likelihood fo the
        measured flux ratios given a model."""
