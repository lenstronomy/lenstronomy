...
class Cosmo(object):
    ...
    def __init__(self, d_d, d_s, d_ds):
        ...
        self.dd = float(
            d_d
        )  # angular diameter distance from observer to deflector in physical Mpc
        self.ds = float(
            d_s
        )  # angular diameter distance from observer to source in physical Mpc
        self.dds = float(
            d_ds
        )  # angular diameter distance from deflector to source in physical Mpc
    ...
    def epsilon_crit(self):
        """Returns the critical projected mass density in units of M_sun/Mpc^2
        (physical units)"""
