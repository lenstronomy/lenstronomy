...
class TestMultiPlaneOrganizer(object):
    ...
    def setup(self):
        ...
        """
        self.lens_redshift_list = [0.2, 0.4, 0.6, 1.]
        self.source_redshift_list = [0.6, 1., 1., 1.2]
        ...
        self.mapping_fiducial = Image2SourceMapping(self.lens_model_fiducial,
                                               self.source_model)
    ...
    def test_extract_a_b_factors(self):
        ...
        """
        kwargs_special  = {'a_factor_1': 1., 'a_factor_2': 2.,
                           'a_factor_3': 3., 'a_factor_4': 4.,
                           'b_factor_2': 5., 'b_factor_3': 6.}

        a_factor_list, b_factor_list = self.multi_plane_organizer._extract_a_b_factors(kwargs_special)
        npt.assert_almost_equal(a_factor_list, [1., 2., 3., 4.])
        npt.assert_almost_equal(b_factor_list, [1., 5., 6., 4.])
    ...
    def test_update_lens_T_lists(self):
        ...
        fiducial_T_ij_start = copy.deepcopy(
            self.lens_model.lens_model.T_ij_start)
        fiducial_T_ij_stop = copy.deepcopy(
            self.lens_model.lens_model._T_ij_stop)

        self.multi_plane_organizer.update_lens_T_lists(self.lens_model,
                                                       kwargs_special)

        npt.assert_almost_equal(self.lens_model.lens_model.multi_plane_base.T_z_list,
                                fiducial_T_z_list, decimal=5)
        npt.assert_almost_equal(self.lens_model.lens_model.multi_plane_base.T_ij_list,
                                fiducial_T_ij_list, decimal=5)
        npt.assert_almost_equal(self.lens_model.lens_model.T_ij_start,
                                fiducial_T_ij_start, decimal=5)
        npt.assert_almost_equal(self.lens_model.lens_model._T_ij_stop,
                                fiducial_T_ij_stop, decimal=5)
    ...
    def test_update_source_mapping_T_lists(self):
        ...
        fiducial_T_ij_start_list = copy.deepcopy(
            self.mapping.T_ij_start_list)
        fiducial_T_ij_end_list = copy.deepcopy(
            self.mapping.T_ij_end_list)

        self.multi_plane_organizer.update_source_mapping_T_lists(self.mapping,
                                                         kwargs_special)

        npt.assert_almost_equal(self.mapping.T_ij_start_list[:2],
                                fiducial_T_ij_start_list[:2], decimal=5)
        assert self.mapping.T_ij_start_list[2] == None
        assert self.mapping.T_ij_start_list[3] == None

        npt.assert_almost_equal(self.mapping.T_ij_end_list,
                                fiducial_T_ij_end_list, decimal=5)
    ...
    def test_get_lens_T_lists(self):
        ...
        npt.assert_almost_equal(T_z_list,
                                [751.3399926289621, 1409.1346240906996, 1982.1199934375895, 2919.3681790661544])

        npt.assert_almost_equal(T_ij_list,
                                [751.3399926289621, 657.7946314617373, 572.9853693468906, 937.2481856285648])
    ...
    def test_get_D_ij(self):
        ...
        D_ij = self.multi_plane_organizer._get_D_ij(
            self.lens_redshift_list[1],
            self.lens_redshift_list[2],
            kwargs_special)
        assert D_ij == 358.1158558418066

        D_ij = self.multi_plane_organizer._get_D_ij(
            self.source_redshift_list[2],
            self.source_redshift_list[3],
            kwargs_special)
        ...
        with pytest.raises(ValueError):
            D_ij = self.multi_plane_organizer._get_D_ij(
                0.1, 0.2, kwargs_special)

        # test if the redshifts are not consecutive
        with pytest.raises(AssertionError):
            D_ij = self.multi_plane_organizer._get_D_ij(
                self.lens_redshift_list[0],
                self.lens_redshift_list[3],
                kwargs_special)
    ...
    def test_get_D_i(self):
        ...
        assert (self.multi_plane_organizer._get_D_i(np.max(self.source_redshift_list),
                                                    kwargs_special
                                                    ) ==
    ...
    def test_transver_distance_start_stop(self):
        ...
        T_ij_start, T_ij_stop = self.multi_plane_organizer._transverse_distance_start_stop(
            self.lens_redshift_list[1],
            self.source_redshift_list[-2],
            kwargs_special)
    ...
    def test_get_source_T_start_end_lists(self):
        ...
        T_ij_start_list, T_ij_end_list = self.multi_plane_organizer._get_source_T_start_end_lists(
            kwargs_special)

        npt.assert_almost_equal(T_ij_start_list[:2],
                                [751.3399926289621, 937.2481856285648])
        assert T_ij_start_list[2] == None
        assert T_ij_start_list[3] == None

        npt.assert_almost_equal(T_ij_end_list,
                                [0.0, 0.0, 0.0, 385.6823981])

    def test_start_condition(self):
        """
        Test MultiPlaneOrganizer._start_condition()
        """
        assert self.multi_plane_organizer._start_condition(True, 1., 1.) == True
        assert self.multi_plane_organizer._start_condition(False, 1., 1.) == False

    def test_distance_computations(self):
        """
        Test the distance computations between with and without distance ratio sampling
        """
        kwargs_lens = [
