...
class TestGaussian(object):
    ...
    def test_function(self):
        ...
        assert values == np.exp(-1.0 / 2)
        x = np.array([2, 3, 4])
        y = np.array([1, 1, 1])
        values = self.Gaussian.function(x, y, amp, center_x, center_y, sigma_x, sigma_y)
        assert values[0] == np.exp(-1.0 / 2)
        assert values[1] == np.exp(-(2.0**2) / 2)
        assert values[2] == np.exp(-(3.0**2) / 2)
    ...
    def test_derivatives(self):
        ...
        sigma_x = 1.0
        sigma_y = 1.0
        values = self.Gaussian.derivatives(
            x, y, amp, center_x, center_y, sigma_x, sigma_y
        )
        assert values[0] == 0.0
        assert values[1] == -np.exp(-1.0 / 2)
        x = np.array([2, 3, 4])
        y = np.array([1, 1, 1])
        values = self.Gaussian.derivatives(
            x, y, amp, center_x, center_y, sigma_x, sigma_y
        )
        assert values[0][0] == -np.exp(-1.0 / 2)
        assert values[1][0] == 0.0
        assert values[0][1] == -2 * np.exp(-(2.0**2) / 2)
    ...
    def test_hessian(self):
        ...
        assert values[3][0] == -np.exp(-1.0 / 2)
        assert values[1][0] == 0.0
