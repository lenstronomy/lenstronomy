...
class TestPixelatedSourceReconstruction(object):
    ...
    def test_generate_M_b_print(self, capsys):
        ...
        assert "number of source pixels: 6 (x axis: 2 pixels;  y axis: 3 pixels)" in captured3.out
        assert ("Step 1: Lensing the source pixels\n"
                "Step 1: Finished!\n"
                "Step 2: Convolve the lensed pixels\n"
                "Step 2: Finished!\n"
                "Step 3: Compute the matrix M and vector b\n"
                "Step 3: Finished!") in captured3.out
        ...
        psr.generate_M_b_diagonal_likelihood(kwargs_lens = self.kwargs_lens, show_progress = False)
        ...
        assert ("Step 1: Lensing the source pixels\n"
                ...
                "Step 2: Finished!\n"
                "Step 3: Compute the matrix M and vector b\n"
                "Step 3: Finished!") in captured6.out
        ...
        psr.generate_M_b_interferometry_natwt_likelihood(kwargs_lens = self.kwargs_lens, show_progress = False)
        captured8 = capsys.readouterr()
        assert captured8.out == ""
        assert captured8.err == ""
        
        psr.generate_M_b_interferometry_natwt_likelihood(kwargs_lens = self.kwargs_lens, verbose = True)
        captured9 = capsys.readouterr()
        assert ("Step 1: Lensing the source pixels\n"
                "Step 1: Finished!\n"
                "Step 2: Compute the matrix M and vector b (including the convolution step)\n"
                "Step 2: Finished!") in captured9.out
    ...
    def test_generate_M_b_diagonal_likelihood(self):
        ...
        psr_noise_map = PixelatedSourceReconstruction(data_class_noise_map_test, self.psf_class,
                                                      self.lens_model_class, self.source_pixel_grid_class)
        M_result1, b_result1 = psr_noise_map.generate_M_b_diagonal_likelihood(kwargs_lens = self.kwargs_lens)
        M_expected1 = np.array([[0.33640217, 0.39787864, 0.40406602, 0.63340722, 0.43471119, 0.69335783],
                                ...
                                [0.63340722, 0.79647494, 0.81155358, 1.32301485, 0.95590111, 1.58523514],
                                [0.43471119, 0.51927341, 0.64754447, 0.95590111, 0.8893692 , 1.36389677],
                                [0.69335783, 0.89520734, 0.99922016, 1.58523514, 1.36389677, 2.21823435]])
        b_expected1 = np.array([ 4.64157473,  6.23768625,  6.35424423, 10.48371933,  8.43403426, 14.16153464])
