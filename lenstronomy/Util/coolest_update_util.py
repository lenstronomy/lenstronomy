...
def shear_update(shear_idx, kwargs_lens, kwargs_lens_mcmc=None):
    """
    Update the COOLEST SHEAR mass model (gamma1 - gamma2) with results in kwargs_lens

    INPUT
    -----
    shear_idx  : coolest.api.profiles.mass.ExternalShearAngleStrength object
    kwargs_lens : dictionnary with the point estimate
...
def pemd_update(mass, kwargs_lens, kwargs_lens_mcmc=None):
    ...
    mass.parameters['center_x'].set_point_estimate(PointEstimate(-float(kwargs_lens['center_x'])))
    mass.parameters['center_y'].set_point_estimate(PointEstimate(float(kwargs_lens['center_y'])))

    if kwargs_lens_mcmc is not None:
        te = [arg['theta_E'] for arg in kwargs_lens_mcmc]
        te_mean = np.mean(te)
        te_16, te_50, te_84 = np.quantile(te, [0.16, 0.5, 0.84])
        mass.parameters['theta_E'].set_posterior(PosteriorStatistics(float(te_mean), float(te_50),
                                                                     float(te_16), float(te_84)))

        g = [arg['gamma'] for arg in kwargs_lens_mcmc]
        g_mean = np.mean(g)
        g_16, g_50, g_84 = np.quantile(g, [0.16, 0.5, 0.84])
        mass.parameters['gamma'].set_posterior(PosteriorStatistics(float(g_mean), float(g_50),
                                                                   float(g_16), float(g_84)))
        ...
        mass.parameters['q'].set_posterior(PosteriorStatistics(float(ql_mean), float(ql_50),
                                                               float(ql_16), float(ql_84)))

        phil_mean = np.mean(phil)
        phil_16, phil_50, phil_84 = np.quantile(phil, [0.16, 0.5, 0.84])
        mass.parameters['phi'].set_posterior(PosteriorStatistics(float(phil_mean), float(phil_50),
                                                                 float(phil_16), float(phil_84)))

        cx = [arg['center_x'] for arg in kwargs_lens_mcmc]
        cx_mean = np.mean(cx)
        cx_16, cx_50, cx_84 = np.quantile(cx, [0.16, 0.5, 0.84])
        mass.parameters['center_x'].set_posterior(PosteriorStatistics(-float(cx_mean), -float(cx_50),
                                                                      -float(cx_16), -float(cx_84)))

        cy = [arg['center_y'] for arg in kwargs_lens_mcmc]
        cy_mean = np.mean(cy)
        cy_16, cy_50, cy_84 = np.quantile(cy, [0.16, 0.5, 0.84])
        mass.parameters['center_y'].set_posterior(PosteriorStatistics(float(cy_mean), float(cy_50),
                                                                      float(cy_16), float(cy_84)))
...
def sersic_update(light, kwargs_light, kwargs_light_mcmc=None):
    ...
    kwargs_light : dictionnary with the point estimate
    ...
    light.parameters['center_y'].set_point_estimate(PointEstimate(float(kwargs_light['center_y'])))

    if kwargs_light_mcmc is not None:
        a = [arg['amp'] for arg in kwargs_light_mcmc]
        a_mean = np.mean(a)
        a_16, a_50, a_84 = np.quantile(a, [0.16, 0.5, 0.84])
        light.parameters['I_eff'].set_posterior(PosteriorStatistics(float(a_mean), float(a_50),
                                                                    float(a_16), float(a_84)))

        rs = [arg['R_sersic'] for arg in kwargs_light_mcmc]
        rs_mean = np.mean(rs)
        rs_16, rs_50, rs_84 = np.quantile(rs, [0.16, 0.5, 0.84])
        light.parameters['theta_eff'].set_posterior(PosteriorStatistics(float(rs_mean), float(rs_50),
                                                                        float(rs_16), float(rs_84)))

        ns = [arg['n_sersic'] for arg in kwargs_light_mcmc]
        ns_mean = np.mean(ns)
        ns_16, ns_50, ns_84 = np.quantile(ns, [0.16, 0.5, 0.84])
        light.parameters['n'].set_posterior(PosteriorStatistics(float(ns_mean), float(ns_50),
                                                                float(ns_16), float(ns_84)))
        ...
        ql_mean = np.mean(ql)
        ql_16, ql_50, ql_84 = np.quantile(ql, [0.16, 0.5, 0.84])
        light.parameters['q'].set_posterior(PosteriorStatistics(float(ql_mean), float(ql_50),
                                                                float(ql_16), float(ql_84)))

        phil_mean = np.mean(phil)
        phil_16, phil_50, phil_84 = np.quantile(phil, [0.16, 0.5, 0.84])
        light.parameters['phi'].set_posterior(PosteriorStatistics(float(phil_mean), float(phil_50),
                                                                  float(phil_16), float(phil_84)))

        cx = [arg['center_x'] for arg in kwargs_light_mcmc]
        cx_mean = np.mean(cx)
        cx_16, cx_50, cx_84 = np.quantile(cx, [0.16, 0.5, 0.84])
        light.parameters['center_x'].set_posterior(PosteriorStatistics(-float(cx_mean), -float(cx_50),
                                                                       -float(cx_16), -float(cx_84)))

        cy = [arg['center_y'] for arg in kwargs_light_mcmc]
        cy_mean = np.mean(cy)
        cy_16, cy_50, cy_84 = np.quantile(cy, [0.16, 0.5, 0.84])
        light.parameters['center_y'].set_posterior(PosteriorStatistics(float(cy_mean), float(cy_50),
                                                                       float(cy_16), float(cy_84)))
...
def shapelets_update(light, kwargs_light, kwargs_light_mcmc=None):
    ...
    if kwargs_light_mcmc is not None:
        ...
        light.parameters['beta'].set_posterior(PosteriorStatistics(float(b_mean), float(b_50),
                                                                   float(b_16), float(b_84)))

        nmax = [arg['n_max'] for arg in kwargs_light_mcmc]
        nmax_mean = np.mean(nmax)
        nmax_16, nmax_50, nmax_84 = np.quantile(nmax, [0.16, 0.5, 0.84])
        light.parameters['n_max'].set_posterior(PosteriorStatistics(float(nmax_mean), float(nmax_50),
                                                                    float(nmax_16), float(nmax_84)))
...
def lensed_point_source_update(light, kwargs_ps, kwargs_ps_mcmc=None):
    ...
    kwargs_ps : dictionnary with the point estimate

    OUTPUT
    ------
     - : updated light
    """
    light.parameters['amps'].set_point_estimate(PointEstimate(np.ndarray.tolist(kwargs_ps['point_amp'])))
    light.parameters['ra_list'].set_point_estimate(PointEstimate(np.ndarray.tolist(-kwargs_ps['ra_image'])))
    light.parameters['dec_list'].set_point_estimate(PointEstimate(np.ndarray.tolist(kwargs_ps['dec_image'])))

    if kwargs_ps_mcmc is not None:
        a = [arg['point_amp'] for arg in kwargs_ps_mcmc]
        a_mean = np.mean(a, axis=0)
        a_16, a_50, a_84 = np.quantile(a, [0.16, 0.5, 0.84], axis=0)
        light.parameters['amps'].set_posterior(PosteriorStatistics(np.ndarray.tolist(a_mean),
        ...
        light.parameters['ra_list'].set_posterior(PosteriorStatistics(np.ndarray.tolist(-ra_mean),
        ...
        light.parameters['dec_list'].set_posterior(PosteriorStatistics(np.ndarray.tolist(dec_mean),
