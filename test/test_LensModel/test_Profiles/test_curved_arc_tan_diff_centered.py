...
class TestCurvedArcTanDiffCentered(object):
    """Tests the source model routines."""

    def setup_method(self):
        self.model = CurvedArcTanDiffCentered()
        self.sie = SIE()
    ...
    def test_curved_arc_round_recovery(self):
        ...
        radial_stretch = 1.0
        center_x_lens, center_y_lens = 0, 0
        dtan_dtan = 0
        kwargs_lens = {
            "tangential_stretch": tangential_stretch,
            "radial_stretch": radial_stretch,
            "dtan_dtan": dtan_dtan,
            "center_x_lens": center_x_lens,
            "center_y_lens": center_y_lens,
            "center_x": center_x,
            "center_y": center_y,
        }
    ...
    def test_curved_arc_recovery(self):
        ...
        kwargs_lens = {
            "tangential_stretch": 2.0,
            "radial_stretch": 1.0,
            "center_x_lens": center_x_lens - 0.5,
            "center_y_lens": center_y_lens,
            "dtan_dtan": 0.1,
            "center_x": center_x,
            "center_y": center_y,
    ...
    def _test_in_out_scaling(self):
        ...
        for dtan_dtan in dtan_dtan_list:
            ...
            (
                ...
                dphi_rad_drad,
                dphi_rad_dtan,
            ) = ext.radial_tangential_differentials(1, 0, [kwargs_lens])
    ...
    def _test_curved_arc_recovery(self, kwargs_arc_init):
        ext = LensModelExtensions(
            LensModel(lens_model_list=["CURVED_ARC_TAN_DIFF_CENTERED"])
        )
        ...
        (
            ...
            center_y,
            [kwargs_arc_init],
            center_x=kwargs_arc_init["center_x_lens"],
            center_y=kwargs_arc_init["center_y_lens"],
        )
        print(lambda_tan, dlambda_tan_dtan, kwargs_arc_init["dtan_dtan"])
        npt.assert_almost_equal(
            kwargs_arc["tangential_stretch"] / kwargs_arc_init["tangential_stretch"],
            1,
            decimal=3,
        )
        npt.assert_almost_equal(
        ...
        curvature = 1 / np.sqrt(
            (kwargs_arc_init["center_x"] - kwargs_arc_init["center_x_lens"]) ** 2
            + (kwargs_arc_init["center_y"] - kwargs_arc_init["center_y_lens"]) ** 2
        )
        direction = np.arctan2(
            kwargs_arc_init["center_y"] - kwargs_arc_init["center_y_lens"],
            kwargs_arc_init["center_x"] - kwargs_arc_init["center_x_lens"],
        )

        npt.assert_almost_equal(kwargs_arc["curvature"], curvature, decimal=2)
        npt.assert_almost_equal(dphi_tan_dtan, curvature, decimal=2)
        npt.assert_almost_equal(kwargs_arc["direction"], direction, decimal=3)
        npt.assert_almost_equal(
            dlambda_tan_dtan / lambda_tan, kwargs_arc_init["dtan_dtan"], decimal=2
        )
