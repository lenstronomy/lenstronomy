...
def auto_raytracing_grid_size(source_fwhm_parcsec, grid_size_scale=0.005, power=1.0):
    ...
    appropriate for magnification computations with finite-size background
    sources. This fit is calibrated for source sizes (interpreted as the FWHM
    of a Gaussian) in the range 0.1 -100 pc.

    :param source_fwhm_parcsec: the full width at half max of a Gaussian
        background source
    :return: an appropriate grid size for finite-size background
...
def setup_mag_finite(grid_radius_arcsec, grid_resolution, source_model, kwargs_source):
    ...
    This new updates allows for more flexibility in the source light
    model by requiring the user to specify the source light mode, grid
    size and grid resolution before calling the function.

    The functions auto_raytrracing_grid_size and
    auto_raytracing_grid_resolution give good estimates for appropriate
    parameter choices for grid_radius_arcsec and grid_resolution.

    :param grid_radius_arcsec: the size of the ray tracing region in
        arcsec; if not specified, an appropriate value will be estimated
        from the source size
    :param grid_resolution: the grid resolution in units arcsec/pixel;
        if not specified, an appropriate value will be estimated from
        the source size
    :param source_model: instance of LightModel for the source
        :kwargs_source: keyword arguments for the light profile
        corresponding to the desired LightModel instance
    """
