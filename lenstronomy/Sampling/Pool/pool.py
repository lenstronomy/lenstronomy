...
def choose_pool(mpi=False, processes=1, **kwargs):
    ...
    It handles the `use_dill` parameters in kwargs, that would otherwise raise an error
    when processes > 1. Any thread in the returned multiprocessing pool (e.g. processes
    > 1) also default

    The requirement of schwimmbad relies on the master branch (as specified in
    requirements.txt). The 'use_dill' functionality can raise if not following the
    ...
    :param processes: Use the multiprocessing pool,
        :class:`~schwimmbad.multiprocessing.MultiPool`, with this number of processes.
        By default, ``processes=1``, will use
        them:class:`~schwimmbad.serial.SerialPool`.
    :type processes: int, optional
    :param kwargs: Any additional kwargs are passed in to the pool class initializer
