...
class Uldm_m_Likelihood(object):
    ...
    """
    def __init__(self, mass_fixed, sigma_mass, lens_model_class, point_source_class):
        """

        :param mass: log10(mass), with mass the mass of the ULDM particle in eV
        :param sigma: uncertainty given to the range of masses one can assume (in the module, it is multiplied by 10**(mass) in order to have a sigma of the order of the mass; choose sigma_mass of the order of 0.01 to have a sharp prior)
        ...
        if lens_model_class.lens_model_list[0] is not 'ULDM-BAR':
            raise ValueError("lens model need to be ULDM-BAR to evaluate the Uldm_m_Likelihood.")
        self._mass_fixed = 10**(mass_fixed)
        self._sigma_mass = sigma_mass * 10**(mass_fixed)
