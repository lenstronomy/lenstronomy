...
def create_lenstronomy_from_coolest(file_name, use_epl=True, **kwargs_serializer):
    ...
    :param file_name: str, name (with path) of the .json file containing
        the COOLEST information
    :param use_epl: bool, if True the elliptical power-law profile is
        'EPL' instead of 'PEMD'
    :return: return_dict, dictionary with typical lenstronomy kwarg (as
        kwargs_data, kwargs_psf, kwargs_params, kwargs_results,
        kwargs_model etc)
    """
        ...
        if lens_observation.noise is not None:
            if lens_observation.noise.type == "NoiseMap":
                ...
                raise NotImplementedError(
                    f"COOLEST noise type {lens_observation.noise.type} "
    ...
    if lens_coolest.instrument is not None:
        ...
        if lens_instrument.psf is not None:
            if lens_instrument.psf.type == "PixelatedPSF":
                ...
                raise NotImplementedError(
                    f"COOLEST PSF type {lens_instrument.psf.type} "
                    "is not supported in the lenstronomy interface."
                )
    ...
    if lens_coolest.cosmology is not None:
        ...
        if lens_cosmo.astropy_name == "FlatLambdaCDM":
            ...
            raise NotImplementedError(
                f"COOLEST cosmology {lens_cosmo.astropy_name} "
        ...
        for lensing_entity in lensing_entities_list:
            if lensing_entity.type == "Galaxy":
                ...
                if galaxy.redshift > min_redshift:
                    ...
                    for light in light_list:
                        ...
                        if light.type == "Sersic":
                            ...
                            raise NotImplementedError(
                                f"COOLEST light profile {light.type} "
                                "is not supported by lenstronomy interface."
                            )
                ...
                if galaxy.redshift < max_redshift:
                    ...
                    if galaxy.redshift > min_redshift:
                        raise NotImplementedError(
                            "Multi-plane is not yet supported "
                    ...
                    for mass in mass_list:
                        ...
                        if mass.type == "PEMD":
                            ...
                            raise NotImplementedError(
                                f"COOLEST mass profile {mass.type} "
                                "is not supported by lenstronomy interface."
                            )
                ...
                for mass_field_idx in mass_field_list:
                    ...
                    if mass_field_idx.type == "ExternalShear":
                        ...
                        # are currently supported this interface.
                        raise NotImplementedError(
                            f"COOLEST mass field type {lensing_entity.type} "
                            "is not supported by lenstronomy interface."
                        )
...
def update_coolest_from_lenstronomy(
    ...
    **kwargs_serializer,
    ...
    """Function to update a json file already containing a model with the
    results of this model fitting.
        ...
        for lensing_entity in lensing_entities_list:
            if lensing_entity.type == "Galaxy":
                ...
                if galaxy.redshift < max_redshift:
                    ...
                    if galaxy.redshift > min_redshift:
                        raise NotImplementedError(
                            "Multi-plane lensing is not yet supported "
                ...
                raise NotImplementedError(
                    f"COOLEST lensing entity type {lensing_entity.type} "
                    "is not supported by lenstronomy interface."
                )
...
def create_kwargs_mcmc_from_chain_list(
    ...
    :param chain_list: list, output of
        FittingSequence.fitting_sequence()
    ...
    :param idx_chain: int, index of the MCMC chain in the chain_list,
        default is the last one. Can be useful if several PSO and MCMC
        are perfomed in the fitting sequence.
    :param likelihood_threshold: float, likelihood limit (negative)
        underwhich the MCMC point is not considered. Can be useful if a
        few chains are stucked in another (less good) minimum
    :return: kwargs_mcmc, list containing all the relevant MCMC points
        in a userfriendly format (with linear parameters etc)
    """
...
def create_redshift_info(lensing_entities_list):
    """Side fuction to create the minimum, maximum and whole redshift list of
    galaxies in the COOLEST template Note that the redshifts helps knowing
    which galaxy is a lens, or a source, and if multiplane has to be
    considered.
