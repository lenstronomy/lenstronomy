...
def setup_lens_model(lens_model, kwargs_lens, index_lens_split):
    """A method to split a lens model into a piece to vary or optimize, and fixed lens model whose deflection field will
    be interpolated. Note that this method currently only supports splitting the lens system at one particular redshift.

    :param lens_model: an instance of LensModel
    :param kwargs_lens: keyword arguments for the lens model
    :param index_lens_split: a list of indexes corresponding to the deflectors that will be left free to vary; every
    other deflector is assumed to remain static be absorbed into a net deflection field
    :return: an instance of LensModel corresponding to the fixed (static) deflectors, an instance of LensModel
    corresponding to free deflectors, keyword arguments for each lens model, source redshift, the redshift where the
    splitting occurs, and the background cosmology
    """
    ...
    lens_model_free = LensModel(lens_model_list_free, lens_redshift_list=lens_redshift_list_free, multi_plane=True,
                                z_source=z_source, cosmo=cosmo)
    lens_model_fixed = LensModel(lens_model_list_fixed, lens_redshift_list=lens_redshift_list_fixed, multi_plane=True,
                                 z_source=z_source, cosmo=cosmo)
    return lens_model_fixed, lens_model_free, kwargs_lens_fixed, kwargs_lens_free, z_source, z_split, cosmo_bkg
...
def coordinates_and_deflections(lens_model_fixed, lens_model_free, kwargs_lens_fixed, kwargs_lens_free,
                                x_coordinate_arcsec, y_coordinate_arcsec, z_split, z_source, cosmo_bkg):
    ...
    xD, yD, alpha_x_foreground, alpha_y_foreground = lens_model_fixed.lens_model.ray_shooting_partial(
        np.zeros_like(x_coordinate_arcsec), np.zeros_like(y_coordinate_arcsec),
        x_coordinate_arcsec,
        y_coordinate_arcsec,
        0.0, z_split,
        kwargs_lens_fixed)
    ...
    alpha_x_main, alpha_y_main = lens_model_free.alpha(theta_x_main, theta_y_main, kwargs_lens_free)
    ...
    return xD, yD, alpha_x_foreground, alpha_y_foreground, alpha_beta_subx, alpha_beta_suby

def class_setup(lens_model_free, x, y, alpha_x_foreground, alpha_y_foreground, alpha_beta_subx,
                        alpha_beta_suby, z_split, coordinate_type='POINT', interp_points=None,
                        x_image=None, y_image=None):
    """This funciton creates the keyword arguments for a LensModel instance that is the decoupled multi-plane
    approxiamtion for the specified lens model
    ...
    :param alpha_beta_subx: deflection field from halos at redshift > z_split given the initial guess for the keyword
    arguments in lens_model_free
    :param alpha_beta_suby: deflection field from halos at redshift > z_split given the initial guess for the keyword
    arguments in lens_model_free
    :param z_split: redshift at which the lens model is decoupled from the line of sight
    :param coordinate_type: specifies the type of interpolation to use. Options are POINT, GRID, or MULTIPLE_IMAGES.
    POINT specifies a single point at which to compute the interpolation
    GRID specifies the interpolation on a regular grid
    ...
    :param x_image: the x coordinates of the image positions if coordinate_type=='MULTIPLE_IMAGES' [arcsec]
    :param y_image: the y coordinates of the image positions if coordinate_type=='MULTIPLE_IMAGES' [arcsec]
    :return: keyword arguments used to create a LensModel with the decoupled multi-plane approximation
    """
    if coordinate_type == 'GRID':
        from scipy.interpolate import RegularGridInterpolator
        npix = int(len(x) ** 0.5)
        interp_xD = RegularGridInterpolator(interp_points, x.reshape(npix, npix).T,
                                            bounds_error=False, fill_value=None)
        interp_yD = RegularGridInterpolator(interp_points, y.reshape(npix, npix).T,
                                            bounds_error=False, fill_value=None)
        interp_foreground_alpha_x = RegularGridInterpolator(interp_points, alpha_x_foreground.reshape(npix, npix).T,
                                                            bounds_error=False, fill_value=None)
        interp_foreground_alpha_y = RegularGridInterpolator(interp_points, alpha_y_foreground.reshape(npix, npix).T,
                                                            bounds_error=False, fill_value=None)
        interp_deltabeta_x = RegularGridInterpolator(interp_points, alpha_beta_subx.reshape(npix, npix).T,
                                                     bounds_error=False, fill_value=None)
        interp_deltabeta_y = RegularGridInterpolator(interp_points, alpha_beta_suby.reshape(npix, npix).T,
                                                     bounds_error=False, fill_value=None)
        ...
        interp_foreground_alpha_x = NearestNDInterpolator(interp_points, alpha_x_foreground)
        interp_foreground_alpha_y = NearestNDInterpolator(interp_points, alpha_y_foreground)
        ...
        raise Exception('coordinate type must be either GRID, POINT, or MULTIPLE_IMAGES')

    kwargs_decoupled_lens_model = {'x0_interp': interp_xD,
                                       'y0_interp': interp_yD,
                                       ...
                                       'z_split': z_split}
    kwargs_lens_model = {'lens_model_list': lens_model_free.lens_model_list,
                         'lens_redshift_list': lens_model_free.redshift_list,
                         'cosmo': lens_model_free.cosmo, 'multi_plane': True,
                         'z_source': lens_model_free.z_source, 'decouple_multi_plane': True,
