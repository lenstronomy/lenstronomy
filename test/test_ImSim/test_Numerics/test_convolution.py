...
class TestPixelKernelConvolution(object):
    ...
    def test_centroiding(self):
        """This test convolves a Gaussian source centered in a centered pixel
        with a Gaussian and checks whether the pixelated FFT convolution
        returns a centered image as well.
...
class TestSubgridKernelConvolution(object):
    ...
    def test_fft_scipy_static(self):
        ...
        conv_sicpy = SubgridKernelConvolution(
            self.kernel,
        ...
        conv_static = SubgridKernelConvolution(
            self.kernel,
            supersampling_factor,
            supersampling_kernel_size=None,
    ...
    def test_convolve2d(self):
        ...
        npt.assert_almost_equal(
            np.sum(model_subgrid_conv), np.sum(model_conv), decimal=1
        )
        ...
        npt.assert_almost_equal(
            np.sum(model_subgrid_conv), np.sum(model_subgrid_conv_split), decimal=8
        )
        ...
        model_subgrid_conv_split = subgrid_conv_split.convolution2d(self.model_sub)
        npt.assert_almost_equal(
            np.sum(model_subgrid_conv), np.sum(model_subgrid_conv_split), decimal=5
        )
...
class TestMultiGaussianConvolution(object):
    ...
    def test_convolve2d(self):
        ...
        mge_conv = MultiGaussianConvolution(
...
class TestMGEConvolution(object):
    ...
    def test_convolve2d(self):
        ...
        npt.assert_almost_equal(
            image_conv_mge / np.max(image_conv_mg),
            image_conv_mg / np.max(image_conv_mg),
            decimal=2,
        )
        ...
        npt.assert_almost_equal(
            diff_kernel,
            pixel_kernel - mge_conv._mge_conv.pixel_kernel(len(pixel_kernel)),
        )
