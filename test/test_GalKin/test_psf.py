from lenstronomy.GalKin.psf import (
    PSF, _fwhm_from_radial_profile, _radial_profile_from_kernel
)
...
class TestPSF(object):
    ...
    def test_kernel(self):
        ...
        psf = PSF(psf_type="MULTI_GAUSSIAN",
                  amplitudes=np.array([0.6, 0.4]), sigmas=np.array([0.3, 0.8]))
        kernel = psf.convolution_kernel(delta_pix=0.3, num_pix=21)
        npt.assert_almost_equal(np.sum(kernel), 1, decimal=5)
        x, y = util.make_grid(numPix=21, deltapix=0.3)
        kernel_direct = psf.convolution_kernel_grid(x, y).reshape(21, 21)
        ...
        psf = PSF(psf_type="PIXEL", kernel=self.gaussian_kernel,
                  delta_pix=0.3, supersampling_factor=1)
        kernel = psf.convolution_kernel(delta_pix=0.3, num_pix=21)
        npt.assert_almost_equal(np.sum(kernel), 1, decimal=5)
...
class TestRaise(unittest.TestCase):
    ...
    def test_pixel_kernel_not_match_scale(self):
        with self.assertRaises(ValueError, msg="PSF delta_pix does not match kernel pixel scale"):
            kernel = np.zeros((5, 5))
            kernel[2, 2] = 1.0
            psf = PSF(
                psf_type="PIXEL", kernel=kernel,
