...
class TestNumerics(object):
    ...
    def assert_integrals(self, Model, kwargs):
        ...
        npt.assert_almost_equal(density2d / density2d_num, 1.0, decimal=1)
    ...
    def assert_lens_integrals(self, Model, kwargs, pi_convention=True):
        """Checks whether the integral in projection of the density_lens()
        function is the convergence.
        ...
        if bool_mass_3d_lens:
            mass_3d_num = int_profile.mass_enclosed_3d(
                r, kwargs_profile=kwargs, lens_param=True
            )
    ...
    def test_PJaffa_density_deflection(self):
        ...
        'sigma0' and the units in the density profile are ok :return:"""
    ...
    def test_nfw(self):
        kwargs = {"rho0": 1.0, "Rs": 5.0, "center_x": 0, "center_y": 0}
        from lenstronomy.LensModel.Profiles.nfw import NFW as Model

        self.assert_integrals(Model, kwargs)

        kwargs_lens = {"alpha_Rs": 1.0, "Rs": 5.0, "center_x": 0, "center_y": 0}
        self.assert_lens_integrals(Model, kwargs_lens)
    ...
    def test_hernquist(self):
        ...
        kwargs = {"rho0": 1.0, "Rs": 5.0}
        self.assert_integrals(Model, kwargs)
        kwargs = {"sigma0": 1.0, "Rs": 5.0}
    ...
    def test_hernquist_ellipse(self):
        ...
        self.assert_integrals(Model, kwargs)
        kwargs = {"sigma0": 1.0, "Rs": 5.0, "e1": 0, "e2": 0}
        self.assert_lens_integrals(Model, kwargs)
    ...
    def test_sie(self):
        ...
        kwargs = {"rho0": 1.0, "e1": 0, "e2": 0}
        self.assert_integrals(Model, kwargs)
        kwargs_lens = {"theta_E": 1.0, "e1": 0, "e2": 0}
        self.assert_lens_integrals(Model, kwargs_lens)
    ...
    def test_spp(self):
        ...
        kwargs = {"rho0": 10.0, "gamma": 2.2}
        self.assert_integrals(Model, kwargs)

        kwargs = {"rho0": 1.0, "gamma": 2.0}
        self.assert_integrals(Model, kwargs)
        kwargs_lens = {"theta_E": 1.0, "gamma": 2.0}
        self.assert_lens_integrals(Model, kwargs_lens)
    ...
    def test_coreBurk(self):
        ...
        kwargs = {"rho0": 1.0, "Rs": 10, "r_core": 5}
        self.assert_integrals(Model, kwargs)

        kwargs = {"rho0": 1.0, "Rs": 9, "r_core": 11}
        self.assert_integrals(Model, kwargs)
    ...
    def test_cnfw(self):
        ...
        kwargs = {"rho0": 1.0, "Rs": 1, "r_core": 0.5}
        self.assert_integrals(Model, kwargs)
        kwargs_lens = {"alpha_Rs": 1.0, "Rs": 5.0, "r_core": 0.5}
        self.assert_lens_integrals(Model, kwargs_lens)
    ...
    def test_cnfw_ellipse(self):
        ...
        # self.assert_integrals(Model, kwargs)
        kwargs_lens = {"alpha_Rs": 1.0, "Rs": 5.0, "r_core": 0.5, "e1": 0, "e2": 0}
        self.assert_lens_integrals(Model, kwargs_lens)

    def test_cored_density(self):
        from lenstronomy.LensModel.Profiles.cored_density import CoredDensity as Model

        kwargs = {"sigma0": 0.1, "r_core": 6.0}
        self.assert_integrals(Model, kwargs)
        self.assert_lens_integrals(Model, kwargs)

    def test_cored_density_2(self):
        from lenstronomy.LensModel.Profiles.cored_density_2 import (
            CoredDensity2 as Model,
        )
    ...
    def test_cored_density_exp(self):
        from lenstronomy.LensModel.Profiles.cored_density_exp import (
            CoredDensityExp as Model,
        )

        kwargs = {"kappa_0": 0.1, "theta_c": 6.0}
        self.assert_integrals(Model, kwargs)
        self.assert_lens_integrals(Model, kwargs)
    ...
    def test_uldm(self):
        ...
        kwargs = {"kappa_0": 0.1, "theta_c": 6.0, "slope": 7.8}
        self.assert_integrals(Model, kwargs)
        self.assert_lens_integrals(Model, kwargs)
    ...
    def test_splcore(self):
        ...
        kwargs = {"rho0": 1.0, "gamma": 3, "r_core": 0.1}
        self.assert_integrals(Model, kwargs)
        kwargs = {"sigma0": 1.0, "gamma": 3, "r_core": 0.1}
        self.assert_lens_integrals(Model, kwargs)

        kwargs = {"rho0": 1.0, "gamma": 2, "r_core": 0.1}
        self.assert_integrals(Model, kwargs)
        kwargs = {"sigma0": 1.0, "gamma": 2, "r_core": 0.1}
        self.assert_lens_integrals(Model, kwargs)

        kwargs = {"rho0": 1.0, "gamma": 2.5, "r_core": 0.1}
        self.assert_integrals(Model, kwargs)
        kwargs = {"sigma0": 1.0, "gamma": 2.5, "r_core": 0.1}
        self.assert_lens_integrals(Model, kwargs)

    def test_nie(self):
        from lenstronomy.LensModel.Profiles.nie import NIE as Model

        kwargs = {"theta_E": 0.7, "s_scale": 0.3, "e1": 0.0, "e2": 0}
        self.assert_lens_integrals(Model, kwargs)

    def test_chameleon(self):
        from lenstronomy.LensModel.Profiles.chameleon import Chameleon as Model

        kwargs = {"alpha_1": 2.0, "w_c": 0.1, "w_t": 2.0, "e1": 0.0, "e2": 0}
        self.assert_lens_integrals(Model, kwargs)
    ...
    def test_pdpl(self):
        from lenstronomy.LensModel.Profiles.pseudo_double_powerlaw import (
            PseudoDoublePowerlaw as Model,
        )

        kwargs = {"alpha_Rs": 0.7, "Rs": 0.3, "gamma_inner": 1.0, "gamma_outer": 3.2}
        self.assert_lens_integrals(Model, kwargs)
        kwargs = {"alpha_Rs": 0.7, "Rs": 0.3, "gamma_inner": 2.5, "gamma_outer": 3.0}
        self.assert_lens_integrals(Model, kwargs)
