...
class GaussianEllipseKappa(LensProfileBase):
    """This class contains functions to evaluate the derivative and hessian
    matrix of the deflection potential for an elliptical Gaussian convergence.
    ...
    def __init__(self, use_scipy_wofz=True, min_ellipticity=1e-5):
        """Setup which method to use the Faddeeva function and the ellipticity
        limit for spherical approximation.
    ...
    def function(self, x, y, amp, sigma, e1, e2, center_x=0, center_y=0):
        ...
        if isinstance(x_, int) or isinstance(x_, float):
            ...
            for i in range(len(x_)):
                f_.append(self._num_integral(x_[i], y_[i], amp_, sigma_, _p, q))
            return np.array(f_)
    ...
    def hessian(self, x, y, amp, sigma, e1, e2, center_x=0, center_y=0):
        ...
        x^2`, :math:`\\partial^2 f/\\partial y^2`, :math:`\\partial^2/\\partial
        x\\partial y`.
    ...
    def sgn(z):
        ...
        deflection as sugggested by Bray (1984). For current implementation,
    ...
    def sigma_function(self, x, y, q):
        r"""Compute the function :math:`\varsigma (z; q)` from equation (4.12)
        of Shajib (2019).
