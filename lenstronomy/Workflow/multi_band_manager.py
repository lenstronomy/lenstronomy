...
class MultiBandUpdateManager(UpdateManager):
    ...
    def __init__(self, kwargs_model, kwargs_constraints, kwargs_likelihood, kwargs_params, num_bands=0):
        ...
        self._kwargs_lens_fixed_init, _, _, _, _ = self.fixed_kwargs
    ...
    def undo_frame_fixed(self, frame_list):
        ...
        for j in frame_list:
            if self._index_lens_model_list[j] is not None:
                for i in self._index_lens_model_list[j]:
                    self._lens_fixed[i] = self._kwargs_lens_fixed_init[i]
    ...
    def fix_not_computed(self, compute_bands):
        ...
        :param compute_bands: boolean list of lenght of the imagine bands
        :return:
        """
        undo_frame_list = []
        fix_frame_list = []
        for i in range(len(compute_bands)):
