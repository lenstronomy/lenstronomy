...
class TestMGESet(object):
    ...
    def test_function(self):
        ...
        e1, e2 = 0.1, -0.05
        sigma = np.logspace(start=np.log10(sigma_min), stop=np.log10(sigma_max), num=self.n_comp)
        amp = np.ones(self.n_comp)
        flux_mg = self.multi_gauss_ellipse.function(x, y, amp=amp, sigma=sigma, center_x=0, center_y=0, e1=e1, e2=e2)
        flux_mge = self.mge_ellipse.function(x, y, amp=amp, sigma_min=sigma_min, sigma_max=sigma_max,
    ...
    def test_function_split(self):
        ...
        sigma = np.logspace(start=np.log10(sigma_min), stop=np.log10(sigma_max), num=self.n_comp)
        amp = np.ones(self.n_comp)
        flux_mg = self.multi_gauss_ellipse.function_split(x, y, amp=amp, sigma=sigma, center_x=0, center_y=0,
                                                          e1=e1, e2=e2)
        flux_mge = self.mge_ellipse.function_split(x, y, amp=amp, sigma_min=sigma_min, sigma_max=sigma_max,
                                                   center_x=0, center_y=0, e1=e1, e2=e2)
    ...
    def test_light3d(self):
        ...
        sigma = np.logspace(start=np.log10(sigma_min), stop=np.log10(sigma_max), num=self.n_comp)
        amp = np.ones(self.n_comp)
        light_3d = self.multi_gauss_ellipse.light_3d(r, amp=amp, sigma=sigma, e1=e1, e2=e2)
        light_3d_mge = self.mge_ellipse.light_3d(r, amp=amp, sigma_min=sigma_min, sigma_max=sigma_max, e1=e1, e2=e2)
