...
class MultiPlaneDecoupled(MultiPlaneBase):
    ...
    def __init__(self,
        ...
        self._fixed_deflection_plane = [True] * len(z_interp_list) + [False] * len(self._lens_redshift_list)
        self._lens_redshift_list_complete = z_interp_list + self._lens_redshift_list
        self._sorted_redshift_indexes = self._index_ordering(self._lens_redshift_list_complete)
    ...
    def ray_shooting_partial(
            ...
            z_start,
            z_stop,
            kwargs_lens, *args, **kwargs,
    ):
        """Ray-shooting through the lens volume with fixed deflection angles at certina lens planes passed through
        ...
        for i, idex in enumerate(self._sorted_redshift_indexes):
            ...
            if z_lens <= z_stop:
                ...
                if self._fixed_deflection_plane[i]:
                    ...
                    alpha_x = self._alphax_interp[counter_fixed_deflection_plane](angle_at_x, angle_at_y)
                    alpha_y = self._alphay_interp[counter_fixed_deflection_plane](angle_at_x, angle_at_y)
                    counter_fixed_deflection_plane += 1
                else:
                    alpha_x, alpha_y = self._add_deflection(x, y, alpha_x, alpha_y,
                                                            kwargs_lens, counter_kwargs_lens)
