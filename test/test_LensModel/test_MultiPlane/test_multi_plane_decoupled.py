...
from lenstronomy.LensModel.Util.decouple_multi_plane_util import setup_lens_model, setup_grids, \
    coordinates_and_deflections, class_setup
import numpy as np
import pytest
...
class TestMultiPlaneDecoupled(object):
    ...
    def setup_method(self):
        ...
        self.zlens = 0.5
        self.z_source = 2.0
        self.kwargs_lens_true = [{'theta_E': 1.0, 'center_x': 0.0, 'center_y': 0.0, 'e1': 0.2, 'e2': -0.1, 'gamma': 2.0},
        ...
        self.lens_model_true = LensModel(self.lens_model_list,
                               lens_redshift_list=self.lens_redshift_list, multi_plane=True,
    ...
    def test_setup_lens_model(self):
        ...
        lens_model_fixed, lens_model_free, kwargs_lens_fixed, kwargs_lens_free, z_source, z_split, cosmo_bkg = \
            setup_lens_model(self.lens_model_true, self.kwargs_lens_true, index_lens_split)
    ...
    def test_coordinates_and_deflections(self):
        ...
        x_true, y_true, alpha_x_foreground_true, alpha_y_foreground_true = self.lens_model_true.lens_model.\
            ray_shooting_partial(0.0, 0.0, x_coordinate_arcsec, y_coordinate_arcsec, 0.0, z_split, self.kwargs_lens_true)
        ...
        alpha_x_main, alpha_y_main = self.lens_model_true.lens_model._multi_plane_base.func_list[0].\
            derivatives(
            theta_x,
            theta_y,
            **self.kwargs_lens_true[0])
        reduced_to_physical = cosmo_bkg.d_xy(0.0, self.z_source) / cosmo_bkg.d_xy(z_split, z_source)
        alpha_x_main_physical = alpha_x_main*reduced_to_physical
        alpha_y_main_physical = alpha_y_main*reduced_to_physical
        npt.assert_almost_equal(alpha_x_foreground, alpha_x_foreground_true + alpha_x_main_physical)
        npt.assert_almost_equal(alpha_y_foreground, alpha_y_foreground_true + alpha_y_main_physical)

        source_x_true, source_y_true = self.lens_model_true.ray_shooting(x_coordinate_arcsec,
                                                                         y_coordinate_arcsec,
    ...
    def test_class_setup(self):
        ...
        source_x_true, source_y_true = self.lens_model_true.ray_shooting(x_coordinate_arcsec_point,
        ...
        kwargs_class_setup = class_setup(lens_model_free, x, y, alpha_x_foreground, alpha_y_foreground, alpha_beta_subx,
                        alpha_beta_suby, z_split, coordinate_type)
        lens_model_decoupled = LensModel(**kwargs_class_setup)
        beta_x_point, beta_y_point = lens_model_decoupled.ray_shooting(x_coordinate_arcsec_point,
                                                                       y_coordinate_arcsec_point,
                                                                       kwargs_lens_free)
        ...
        source_x_true, source_y_true = self.lens_model_true.ray_shooting(xgrid,
                                                                         ygrid,
                                                                         self.kwargs_lens_true)
        x, y, alpha_x_foreground, alpha_y_foreground, alpha_beta_subx, alpha_beta_suby = \
            coordinates_and_deflections(lens_model_fixed,
        ...
        kwargs_class_setup = class_setup(lens_model_free, x, y, alpha_x_foreground, alpha_y_foreground, alpha_beta_subx,
                                         alpha_beta_suby, z_split, coordinate_type, interp_points=interp_points)
        lens_model_decoupled = LensModel(**kwargs_class_setup)
        beta_x, beta_y = lens_model_decoupled.ray_shooting(x_coordinate_arcsec_point, y_coordinate_arcsec_point,
                                                           kwargs_lens_free)
        npt.assert_almost_equal(beta_x, beta_x_point, 5)
        npt.assert_almost_equal(beta_y, beta_y_point, 5)

        beta_x_grid, beta_y_grid = lens_model_decoupled.ray_shooting(xgrid, ygrid,
                                                           kwargs_lens_free)
        ...
        source_x_true, source_y_true = self.lens_model_true.ray_shooting(x_image,
                                                                         y_image,
                                                                         self.kwargs_lens_true)
        x, y, alpha_x_foreground, alpha_y_foreground, alpha_beta_subx, alpha_beta_suby = \
            coordinates_and_deflections(lens_model_fixed,
        ...
        kwargs_class_setup = class_setup(lens_model_free, x, y, alpha_x_foreground, alpha_y_foreground, alpha_beta_subx,
                                         alpha_beta_suby, z_split, coordinate_type, x_image=x_image, y_image=y_image)
        lens_model_decoupled = LensModel(**kwargs_class_setup)
        beta_x_multiple_images_array, beta_y_multiple_images_array = lens_model_decoupled.ray_shooting(x_image,
                                                                                           y_image,
                                                                                           kwargs_lens_free)
        ...
        beta_x_multiple_images, beta_y_multiple_images = lens_model_decoupled.ray_shooting(x_image[1] + 0.05,
                                                                                           y_image[1] - 0.1,
                                                                                           kwargs_lens_free)
