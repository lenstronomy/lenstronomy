...
class TestPointSource(object):
        ...
        self.kwargs_ps = [
            {
                ...
                "point_amp": np.ones_like(self.x_pos) * 2,
            },
            {"ra_image": [1.0], "dec_image": [1.0], "point_amp": [10]},
            {
    ...
    def test_image_position(self):
        x_image_list, y_image_list = self.PointSource.image_position(
            kwargs_ps=self.kwargs_ps, kwargs_lens=self.kwargs_lens, width_search_factor=1.5
        )
    ...
    def test_linear_response_set(self):
        ...
        ra_pos, dec_pos, amp, n = self.PointSource.linear_response_set(
            self.kwargs_ps, kwargs_lens=self.kwargs_lens, with_amp=True
        )
        num_basis = self.PointSource.num_basis(self.kwargs_ps, self.kwargs_lens)
        assert amp[0][0] != 1
        assert n == num_basis
...
class TestPointSourceFixedMag(object):
    def setup_method(self):
        ...
        self.kwargs_ps = [
            {"ra_image": self.x_pos, "dec_image": self.y_pos, "source_amp": 1},
            {"ra_image": [1.0], "dec_image": [1.0], "point_amp": [10]},
            {
                "ra_source": self.sourcePos_x,
                "dec_source": self.sourcePos_y,
    ...
    def test_check_image_positions(self):
        bool = self.PointSource.check_image_positions(
            self.kwargs_ps, self.kwargs_lens, tolerance=0.001
        )
        assert bool is True

        # now we change the lens model to make the test fail
        kwargs_lens = [
            {"theta_E": 2.0, "center_x": 0, "center_y": 0, "e1": 0, "e2": 0, "gamma": 2}
        ]
        bool = self.PointSource.check_image_positions(
            self.kwargs_ps, kwargs_lens, tolerance=0.001
        )
    ...
    def test_positive_flux(self):
        bool = PointSource.check_positive_flux(
            kwargs_ps=[{"point_amp": np.array([1, -1])}]
        )
        assert bool is False
        ...
        bool = PointSource.check_positive_flux(
            kwargs_ps=[{"point_amp": np.array([0, 1])}]
        )
        assert bool is True
        bool = PointSource.check_positive_flux(kwargs_ps=[{"point_amp": 1}])
        assert bool is True
        ...
        bool = PointSource.check_positive_flux(
            kwargs_ps=[{"point_amp": np.array([0, 1]), "source_amp": 1}]
        )
        assert bool is True
        bool = PointSource.check_positive_flux(
            kwargs_ps=[{"point_amp": 1, "source_amp": -1}]
        )
        assert bool is False
