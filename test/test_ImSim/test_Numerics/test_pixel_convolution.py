...
from lenstronomy.ImSim.Numerics.numba_convolution import (
    NumbaConvolution,
    SubgridNumbaConvolution,
)
...
class TestPixelConvolution(object):
    def setup_method(self):
        ...
        x, y = util.make_grid(num_pix=self.num_pix_kernel, deltapix=self.delta_pix)
        ...
        x, y = util.make_grid(num_pix=self.num_pix, deltapix=self.delta_pix)
    ...
    def test_convolve2d(self):
        ...
        numba_conv = NumbaConvolution(
            kernel=self.kernel, conv_pixels=conv_pixels, compute_pixels=conv_pixels
        )
...
class TestSubgirdNumbaConvolution(object):
    def setup_method(self):
        ...
        self.model = image_util.re_size(
            self.model_super, factor=self.supersampling_factor
        )

    def test_convolve2d(self):
        conv_pixels = np.ones_like(self.model)
        conv_pixels = np.array(conv_pixels, dtype=bool)
        numba_conv = SubgridNumbaConvolution(
            kernel_super=self.kernel_super,
            conv_pixels=conv_pixels,
            compute_pixels=conv_pixels,
        ...
        image_convolved = image_util.re_size(
            image_convolved, factor=self.supersampling_factor
        )
        npt.assert_almost_equal(model_conv_numba, image_convolved, decimal=10)
