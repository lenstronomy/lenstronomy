...
class SynthesisProfile(LensProfileBase):
    ...
    def _linear_weight_mle_fit(self, kwargs_target, kwargs_list):
        ...
        Y = self.target_class.kappa(
            x=self.r_eval_list,
            y=np.zeros_like(self.r_eval_list),
            kwargs=kwargs_target_centered,
        )
        M = np.zeros((len(self.r_eval_list), self.num_components))
        C = np.diag(Y * self.sigma)  # covariance matrix between components
        for j in range(
            self.num_components
        ):  # M[i,j] is the jth component 1D kappa evaluated at r[i].
            kwargs = self.circular_centered_kwargs(kwargs_list[j])
            M[:, j] = self.component_class.kappa(
                x=self.r_eval_list, y=0, kwargs=[kwargs]
            )
    ...
    def set_limits(self, kwargs_list, lin_fit_hyperparams):
        ...
        self.r_eval_list = np.logspace(
            self.lower_log_bound, self.upper_log_bound, self.num_r_evals
        )
    ...
    def function(self, x, y, **kwargs_target):
        ...
        for kwargs, weight in zip(self.kwargs_list, weight_list):
            f_ += weight * self.component_class.potential(
                x, y, [kwargs]
            )  # -potential_offset)
            f_innermost += weight * self.component_class.potential(
                [10**self.lower_log_bound], [0], [kwargs]
            )
    ...
    def hessian(self, x, y, **kwargs_target):
        ...
        for kwargs, weight in zip(self.kwargs_list, weight_list):
            f_xx_i, f_xy_i, f_yx_i, f_yy_i = weight * np.array(
                self.component_class.hessian(x, y, [kwargs])
            )
            f_xx += f_xx_i
            f_xy += f_xy_i
            f_yx += f_yx_i
    ...
    def check_num_evals(self):
        ...
        if num_comp >= self.lin_fit_hyperparams["num_r_evals"]:
            raise ValueError(
                "There must be more num_r_evals than components or the profile will be overfit"
            )
