...
class PsfFitting(object):
    """Class to find subsequently a better psf The method make use of a model
    and subtracts all the non-point source components of the model from the
    data. If the model is sufficient, then the data will be a (better)
    representation of the actual PSF. The method cuts out those point sources
    and combines them to update the estimate of the PSF. This is done in an
    iterative procedure as the model components of the extended features is
    PSF-dependent (hopefully not too much).
    ...
    def calc_cornermask(kernelsize, psf_symmetry):
        """Calculate the completeness numerically when rotational symmetry is
        imposed. This is the simplest 'mask' which throws away anywhere the
        rotations are not fully complete ->e.g. in the corners. This ONLY
        accounts for information loss in corners, not due e.g. to losses at the
        edges of the images.

        :param kernelsize: int, size of kernel array
        :param psf_symmetry: int, the symmetry being imposed on the data
        :return: mask showing where the psf with symmetry n is
            incomplete due to rotation.
        """
    ...
    def image_single_point_source(self, image_model_class, kwargs_params):
        """Return model without including the point source contributions as a
        list (for each point source individually)

        :param image_model_class: ImageModel class instance
        :param kwargs_params: keyword arguments of model component
    ...
    def point_like_source_cutouts(x_pos, y_pos, image_list, cutout_size):
        ...
        :param image_list: list of 2d numpy arrays with cleaned images,
            with all contaminating sources removed except the point-like
            object to be cut out.
    ...
    def combine_psf(
        ...
        :param kernel_list_new: list of new PSF kernels estimated from
            the point sources in the image (un-normalized)
        :param kernel_old: old PSF kernel
        :param factor: weight of updated estimate based on new and old
            estimate, factor=1 means new estimate, factor=0 means old
            estimate
        :param stacking_option: option of stacking, mean or median
        :param symmetry: imposed symmetry of PSF estimate
        :param corner_symmetry: int, if the imposed symmetry is an odd
            number, the edges of the reconstructed PSF in its default
            form will be clipped at the corners. corner_symmetry 1)
            tracks where the residuals are being clipped by the imposed
            symmetry and then 2) creates a psf with symmetry=corner
            symmetry which is either 1 or 360/symm = n*90. (e.g for a
            symmetry 6 psf you could use symmetry 2 in the corners). 3)
            adds the corner_symmetry psf (which has information at the
            corners) to the odd symmetry PSF, in the regions where the
            odd-symmetry PSF does not have complete information.
    ...
    def error_map_estimate_new(
        ...
        """Relative uncertainty in the psf model (in quadrature) per pixel
        based on residuals achieved in the image.

        :param psf_kernel: PSF kernel (super-sampled)
        :param psf_kernel_list: list of individual best PSF kernel
            estimates (super- sampled)
        ...
        :return: psf error map such that square of the uncertainty gets
            boosted by error_map * (psf * amp)**2
        """
    ...
    def error_map_estimate(
        ...
        """Provides a psf_variance_map based on the goodness of fit of the
        given PSF kernel on the point source cutouts, their estimated
        amplitudes and positions.

        :param kernel: PSF kernel
        :param star_cutout_list: list of 2d arrays of cutouts of the
            point sources with all other model components subtracted
        :param amp: list of amplitudes of the estimated PSF kernel
        :param x_pos: pixel position (in original data unit, not in
            cutout) of the point sources (same order as amp and star
            cutouts)
        :param y_pos: pixel position (in original data unit, not in
            cutout) of the point sources (same order as amp and star
            cutouts)
        :param error_map_radius: float, radius (in arc seconds) of the
            outermost error in the PSF estimate (e.g. to avoid double
            counting of overlapping PSF erros)
        :param block_center_neighbour: angle, radius of neighbouring
            point sources around their centers the estimates is ignored.
            Default is zero, meaning a not optimal subtraction of the
            neighbouring point sources might contaminate the estimate.
        :return: relative uncertainty in the psf model (in quadrature)
            per pixel based on residuals achieved in the image
        """
