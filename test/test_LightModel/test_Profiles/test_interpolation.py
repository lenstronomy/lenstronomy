...
class TestInterpol(object):
    ...
    def test_function(self):
        ...
        for len_x, len_y in [(20, 20), (14, 20)]:
            x, y = util.make_grid(num_pix=(len_x, len_y), deltapix=1.0)
            gauss = Gaussian()
            ...
            flux = gauss.function(
                x - 1.0, y, amp=1.0, center_x=0.0, center_y=0.0, sigma=1.0
            )
            kwargs_interp = {
                "image": image,
                "scale": 1.0,
            ...
            flux = gauss.function(
                x - 1.0, y - 1.0, amp=1, center_x=0.0, center_y=0.0, sigma=1.0
            )
            kwargs_interp = {
                "image": image,
                "scale": 1.0,
        ...
        out = interp.function(
            x=0, y=0, image=image, scale=1.0, phi_G=0, center_x=0, center_y=0
        )
        out_shift = interp.function(
            x=1, y=0, image=image, scale=1.0, phi_G=0, center_x=1, center_y=0
        )
        ...
        out = interp.function(
            x=1.0, y=0, image=image, scale=1.0, phi_G=0, center_x=0, center_y=0
        )
        out_scaled = interp.function(
            x=2.0, y=0, image=image, scale=2, phi_G=0, center_x=0, center_y=0
        )
    ...
    def test_flux_normalization(self):
        ...
        delta_pix = 0.1
        len_x, len_y = 21, 21
        x, y = util.make_grid(num_pix=(len_x, len_y), deltapix=delta_pix)
        gauss = Gaussian()
        ...
        scale = 0.5
        x, y = util.make_grid(num_pix=(len_x, len_y), deltapix=delta_pix * scale)
        kwargs_interp = {
            "image": image,
    ...
    def test_delete_cache(self):
        x, y = util.make_grid(num_pix=20, deltapix=1.0)
        gauss = Gaussian()
        flux = gauss.function(x, y, amp=1.0, center_x=0.0, center_y=0.0, sigma=1.0)
