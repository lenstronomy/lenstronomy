...
class PL_Sersic(object):
    """This class contains functions to evaluate a 2D PL-Sérsic surface brightness profile.
    ...
    param_names = ["amp", "R_sersic", "n_sersic", "alpha_c", "r_c", "e1", "e2", "center_x", "center_y"]
    ...
    def calculate_param(self, R_sersic, n_sersic):
        """
        Calculate the scale radius s from the 2D effective radius R_sersic and the Sérsic index n.
        ...
        k = (2 * n_sersic) ** (1 / n_sersic)  # Based on the relationship for k from the literature
    ...
    def function(
        self, x, y, amp, R_sersic, n_sersic, alpha_c, r_c, e1, e2,
        center_x=0, center_y=0, max_R_frac=1000.0
    ):
        """
        ...
        """
        R = self.get_distance_from_center_bpl(
    ...
    def total_flux(self, amp, R_sersic, n_sersic, alpha_c, r_c, e1=0, e2=0, **kwargs):
        ...
        """
        if alpha_c >= 3.0:
            raise ValueError("alpha_c must be < 3 for finite total flux in the inner power-law part.")
