...
class TestTracerModelFit(object):
    ...
    def setup_method(self):
        ...
        kwargs_source_init.append(
            {'R_sersic': 0.2, 'n_sersic': 1, 'e1': 0, 'e2': 0, 'center_x': 0., 'center_y': 0, 'amp': 16})
        kwargs_source_sigma.append(
            {'n_sersic': 0.5, 'R_sersic': 0.1, 'e1': 0.05, 'e2': 0.05, 'center_x': 0.2, 'center_y': 0.2, 'amp': 10})
        kwargs_lower_source.append(
            {'e1': -0.5, 'e2': -0.5, 'R_sersic': 0.001, 'n_sersic': .5, 'center_x': -10, 'center_y': -10, 'amp': 0})
        kwargs_upper_source.append(
            {'e1': 0.5, 'e2': 0.5, 'R_sersic': 10, 'n_sersic': 5., 'center_x': 10, 'center_y': 10, 'amp': 100})

        source_params = [kwargs_source_init, kwargs_source_sigma, fixed_source, kwargs_lower_source,
                         kwargs_upper_source]
        ...
        kwargs_lens_light_sigma.append(
            {'n_sersic': 1, 'R_sersic': 0.3, 'e1': 0.05, 'e2': 0.05, 'center_x': 0.1, 'center_y': 0.1, 'amp': 10})
        ...
        fixed_tracer_source = [{}]
        kwargs_tracer_source_init = [{'amp': 100, 'k': 1, 'center_x': 0., 'center_y': 0}]
        kwargs_tracer_source_sigma = [{'amp': 50, 'k': 1, 'center_x': .2, 'center_y': .2}]
        kwargs_lower_tracer_source = [{'amp': 0, 'k': -2, 'center_x': -5, 'center_y': -5}]
        kwargs_upper_tracer_source = [{'amp': 1000, 'k': 5, 'center_x': 5, 'center_y': 5}]

        tracer_source_params = [kwargs_tracer_source_init, kwargs_tracer_source_sigma, fixed_tracer_source,
                                kwargs_lower_tracer_source, kwargs_upper_tracer_source]
        ...
        kwargs_constraints = {'linear_solver': True,
                              ...
                              'joint_source_light_with_tracer': [[0, 0, ['center_x',
                                                                         'center_y']]]}  # link source light with tracer [[i_source_light, k_tracer_source, ['param_name1', 'param_name2', ...]], [...], ...]
    ...
    def test_run_fit(self):
        ...
        fitting_seq = FittingSequence(self.kwargs_data_joint, self.kwargs_model, self.kwargs_constraints,
                                      self.kwargs_likelihood,
                                      self.kwargs_params)

        fitting_kwargs_list = [
            ['update_settings', {'tracer_source_add_fixed': [[0, ['amp', 'k']]],  # add tracer fixed to avoid sampling
                                 'kwargs_likelihood': {'tracer_likelihood': False}  # remove tracer data to be fitted
                                 }],
            ['PSO', {'sigma_scale': 1., 'n_particles': 10, 'n_iterations': 10}],
            ['update_settings', {'tracer_source_remove_fixed': [[0, ['amp', 'k']]],  # remove tracer fixed to sample
                                 'kwargs_likelihood': {'tracer_likelihood': True}}],  # evaluate tracer data likelihood
            ['PSO', {'sigma_scale': .1, 'n_particles': 10, 'n_iterations': 10}],
        ...
        npt.assert_almost_equal(kwargs_result['kwargs_tracer_source'][0]['k'], 1, decimal=1)
