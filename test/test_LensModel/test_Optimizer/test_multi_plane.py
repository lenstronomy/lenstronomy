...
class TestMultiPlaneOptimizer(object):
    ...
    def setup(self):
        ...
        self.optimizer_simple_variablegamma = Optimizer(self.x_pos_simple, self.y_pos_simple,
                                          magnification_target=self.magnification_simple,
                                          redshift_list=redshift_list_simple,
                                          lens_model_list=lens_model_list_simple, kwargs_lens=self.kwargs_lens_simple,
                                          multiplane=True, verbose=True, z_source=1.5, z_main=0.5,
                                          astropy_instance=self.cosmo, optimizer_routine='variable_powerlaw_shear')
    ...
    def test_split_multiplane_rayshooting(self):
        ...
        betax, betay = split._ray_shooting_fast(macromodel_args=args, thetax=xpos, thetay=ypos, force_compute=True)
    ...
    def test_multi_plane_simple(self):
        ...
        kwargs_lens, source, [x_image, y_image] = self.optimizer_simple_variablegamma.optimize(n_particles=20, n_iterations=10,
                                                                               restart=1)
