...
class MultiBandUpdateManager(UpdateManager):
    ...
    The class inherits the UpdateManager() class and adds
    functionalities to hold and relieve fixed all lens model parameters
    of a specific frame/image for more convenient use of the
    FittingSequence.
    """
    ...
    def fix_not_computed(self, free_bands):
        """Fix all the lens models that are part of a imaging band that is not
        set to be computed. Free those that are modeled. #TODO check for
        overlapping models for more automated fixing of parameters.

        :param free_bands: boolean list of length of the imaging bands,
            True indicates that the lens model is being fitted for
        :return: None
        """
        undo_frame_list = []
        fix_frame_list = []
