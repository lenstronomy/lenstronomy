...
class TestLikelihoodModule(object):
    ...
    def setup_method(self):
        ...
        self.kwargs_data = {'multi_band_list': [[kwargs_band, kwargs_psf, kwargs_numerics]],
        ...
        self.Likelihood = LikelihoodModule(kwargs_data_joint=self.kwargs_data, kwargs_model=kwargs_model,
    ...
    def test_time_delay_likelihood(self):
        ...
        likelihood = LikelihoodModule(kwargs_data_joint=self.kwargs_data, kwargs_model=self.kwargs_model,
                                      param_class=self.param_class, **kwargs_likelihood)
        args = self.param_class.kwargs2args(kwargs_lens=self.kwargs_lens, kwargs_source=self.kwargs_source,
                                            kwargs_lens_light=self.kwargs_lens_light, kwargs_ps=self.kwargs_ps,
                                            kwargs_special=self.kwargs_cosmo)
    ...
    def test_kin_likelihood(self):
        ...
        if self.kinematic_NN:
            ...
            kwargs_sersic_ellipse_qphi = {'amp': 1., 'R_sersic': .51, 'n_sersic': 2.5, 'center_x': 0, 'center_y': 0,
                                          'q': 0.99, 'phi': 0.7}
            ...
            image_averaged_chi2 = (
                        (np.mean(Likelihood.kinematic_2D_likelihood.vrms) - binned_dummy_data) ** 2 / kwargs_kin[
                    'bin_cov']).flatten()
    ...
    def test_pixelbased_modelling(self):
        ...
        kwargs_fixed_source = [
            {'n_scales': n_scales, 'n_pixels': numPix_source ** 2, 'scale': 1, 'center_x': 0, 'center_y': 0}]
        kwargs_fixed_lens_light = [
            {'n_scales': n_scales, 'n_pixels': self.numPix ** 2, 'scale': 1, 'center_x': 0, 'center_y': 0}]
