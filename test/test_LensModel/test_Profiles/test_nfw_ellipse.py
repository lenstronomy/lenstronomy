...
class TestNFWELLIPSE(object):
    ...
    def test_function(self):
        ...
        values_e = self.nfw_e.function(x, y, Rs, theta_Rs, e1, e2)
        ...
        phi_G = 0
        e1, e2 = param_util.phi_q2_ellipticity(phi_G, q)
        values = self.nfw_e.function(x, y, Rs, theta_Rs,e1, e2)
    ...
    def test_derivatives(self):
        ...
        f_x_e, f_y_e = self.nfw_e.derivatives(x, y, Rs, theta_Rs, e1, e2)
        npt.assert_almost_equal(f_x[0], f_x_e[0], decimal=5)
        npt.assert_almost_equal(f_y[0], f_y_e[0], decimal=5)
        x = np.array([0])
        y = np.array([0])
        theta_Rs = 0
        f_x, f_y = self.nfw_e.derivatives(x, y, Rs, theta_Rs, e1, e2)
        ...
        phi_G = 0
        e1, e2 = param_util.phi_q2_ellipticity(phi_G, q)
        values = self.nfw_e.derivatives(x, y, Rs, theta_Rs, e1, e2)
    ...
    def test_hessian(self):
        ...
        f_xx_e, f_yy_e, f_xy_e = self.nfw_e.hessian(x, y, Rs, theta_Rs, e1, e2)
        ...
        phi_G = 0
        e1, e2 = param_util.phi_q2_ellipticity(phi_G, q)
        values = self.nfw_e.hessian(x, y, Rs, theta_Rs, e1, e2)
