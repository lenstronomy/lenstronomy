...
class LightCone(object):
    """Class to perform multi-plane ray-tracing from convergence maps at
    different redshifts From the convergence maps the deflection angles and
    lensing potential are computed (from different settings) and then an
    interpolated grid of all those quantities generate an instance of the
    lenstronomy LensModel multi-plane instance. All features of the LensModel
...
class MassSlice(object):
    ...
    def interpol_instance(self, z_source, cosmo):
        """Scales the mass map integrals (with units of mass not convergence)
        into a convergence map for the given cosmology and source redshift and
        returns the keyword arguments of the interpolated reduced deflection
        and lensing potential.

        :param z_source: redshift of the source
        :param cosmo: astropy.cosmology instance
        :return: keyword arguments of the interpolation instance with
            numerically computed deflection angles and lensing potential
        """
