...
class TestEllipticalMultipole(object):
    ...
    def test_function(self):
        ...
        values = self.Multipole.function(x, y, m, a_m, varphi_m, q, phi_ref = 0.)
        ...
        xx_nonrot, yy_nonrot = rr*np.cos(phi), rr*np.sin(phi) #coordinates in non-rotated frame
        xx_rot, yy_rot = rr*np.cos(phi-phi_rot_test), rr*np.sin(phi-phi_rot_test) #coordinates in non-rotated frame
        ...
        values_rot = self.Multipole.function( #values in rotated frame
            xx_rot, yy_rot, m, a_m, varphi_m, q, phi_ref = 0.
        )
        ...
        function_ell_limit = self.Multipole.function(
            self.x, self.y, m=4, a_m=a_m, varphi_m=np.pi / 24, q=0.9995, phi_ref = 0.
        )
        ...
        function_ell_limit = self.Multipole.function(
            self.x, self.y, m=3, a_m=a_m, varphi_m=np.pi / 18, q=0.9999, phi_ref = 0.
        )
        function_circ = self.CircularMultipole.function(
        ...
        function_ell_limit = self.Multipole.function(
            self.x, self.y, m=1, a_m=a_m, varphi_m=np.pi / 6, q=0.9999999, phi_ref = 0.
        )
        function_circ = self.CircularMultipole.function(
    ...
    def test_derivatives(self):
        ...
        a_m = 0.05
        varphi_m = 25 * np.pi / 180.0
        f_x, f_y = self.Multipole.derivatives(x, y, m, a_m, varphi_m, q, phi_ref = 0.)
    ...
    def test_hessian(self):
        ...
        a_m = 0.05
        varphi_m = 25 * np.pi / 180.0
        f_xx, f_xy, f_yx, f_yy = self.Multipole.hessian(x, y, m, a_m, varphi_m, q, phi_ref = 0.)
        ...
        x = np.array([2, 3, 4])
        y = np.array([1, 1, 1])
        values = self.Multipole.hessian(x, y, m, a_m, varphi_m, q, phi_ref = 0.)
        ...
        npt.assert_almost_equal(
            (values[0] + values[3]), a_m * np.cos(m * (phi_ell - varphi_m)) / R, decimal=6
        )
        ...
        xx_nonrot, yy_nonrot = rr*np.cos(phi), rr*np.sin(phi) #coordinates in non-rotated frame
        xx_rot, yy_rot = rr*np.cos(phi-phi_rot_test), rr*np.sin(phi-phi_rot_test) #coordinates in non-rotated frame
        f_xx_nonrot, f_xy_nonrot, f_yx_nonrot, f_yy_nonrot = Multipole.hessian( #values in non-rotated frame
            xx_nonrot, yy_nonrot, m, a_m, varphi_m, q, phi_ref = phi_rot_test
        ) 
        f_xx_rot, f_xy_rot, f_yx_rot, f_yy_rot = Multipole.hessian( #values in rotated frame
            xx_rot, yy_rot, m, a_m, varphi_m, q, phi_ref = 0.
        ) 
        ...
        gamma1_rerot, gamma2_rerot = util.rotate(gamma1_nonrot, gamma2_nonrot, 2*phi_rot_test) #shear is a spin-2 tensor
        ...
        f_xx_ell_q1, f_xy_ell_q1, f_yx_ell_q1, f_yy_ell_q1 = self.Multipole.hessian(
            self.x, self.y, m=4, a_m=a_m, varphi_m=np.pi / 24, q=1.0, phi_ref = 0.
        )
        ...
        f_xx_ell_q1, f_xy_ell_q1, f_yx_ell_q1, f_yy_ell_q1 = self.Multipole.hessian(
            self.x, self.y, m=3, a_m=a_m, varphi_m=np.pi / 18, q=1.0, phi_ref = 0.
        )
        npt.assert_almost_equal(f_xx_ell_q1, f_xx_circ, decimal=8)
        npt.assert_almost_equal(f_xy_ell_q1, f_xy_circ, decimal=8)
        npt.assert_almost_equal(f_yx_ell_q1, f_yx_circ, decimal=8)
        ...
        f_xx_ell_q1, f_xy_ell_q1, f_yx_ell_q1, f_yy_ell_q1 = self.Multipole.hessian(
            self.x, self.y, m=1, a_m=a_m, varphi_m=np.pi / 6, q=1.0, phi_ref = 0.
        )
        npt.assert_almost_equal(f_xx_ell_q1, f_xx_circ, decimal=8)
        npt.assert_almost_equal(f_xy_ell_q1, f_xy_circ, decimal=8)
        npt.assert_almost_equal(f_yx_ell_q1, f_yx_circ, decimal=8)
