...
from lenstronomy.LensModel.Util.decouple_multi_plane_util import (
    setup_lens_model,
    setup_grids,
    coordinates_and_deflections, setup_raytracing_lensmodels,
    class_setup
)
...
class TestMultiPlaneDecoupled(object):
    ...
    def setup_method(self):
        ...
        self.reduced_to_phys = self.cosmo_bkg.d_xy(0, self.z_source) / self.cosmo_bkg.d_xy(self.zlens, self.z_source)
    ...
    def _setup_point(self):
        ...
        self.x0_point, self.y0_point, self.alphax_foreground_point, self.alphay_foreground_point, \
        self.alphax_background_point, self.alphay_background_point = coordinates_and_deflections(self.lens_model_fixed,
                                                                                     self.lens_model_free,
                                                                                     self.kwargs_lens_fixed,
                                                                                     self.kwargs_lens_free,
                                                                                     self.x_image[1],
        ...
        self.kwargs_multiplane_model_point = class_setup(self.lens_model_free, self.x0_point, self.y0_point,
                                                         self.alphax_foreground_point, self.alphay_foreground_point,
                                                         self.alphax_background_point, self.alphay_background_point,
                                                         self.z_split,
    ...
    def _setup_grid(self):
        ...
        self.grid_x, self.grid_y, self.interp_points_grid, self.npix_grid = setup_grids(grid_size, grid_resolution, 0.0, 0.0)
        self.x0_grid, self.y0_grid, self.alphax_foreground_grid, self.alphay_foreground_grid, \
        self.alphax_background_grid, self.alphay_background_grid = coordinates_and_deflections(self.lens_model_fixed,
                                                                                     self.lens_model_free,
                                                                                     self.kwargs_lens_fixed,
                                                                                     self.kwargs_lens_free,
                                                                                     self.grid_x,
        ...
        self.kwargs_multiplane_model_grid = class_setup(self.lens_model_free, self.x0_grid, self.y0_grid,
                                                         self.alphax_foreground_grid, self.alphay_foreground_grid,
                                                         self.alphax_background_grid, self.alphay_background_grid,
                                                         self.z_split,
    ...
    def _setup_multiple_images(self):
        ...
        self.x0_MI, self.y0_MI, self.alphax_foreground_MI, self.alphay_foreground_MI, \
        self.alphax_background_MI, self.alphay_background_MI = coordinates_and_deflections(self.lens_model_fixed,
                                                                                     self.lens_model_free,
                                                                                     self.kwargs_lens_fixed,
                                                                                     self.kwargs_lens_free,
                                                                                     self.x_image,
        ...
        self.kwargs_multiplane_model_MI = class_setup(self.lens_model_free, self.x0_MI, self.y0_MI,
                                                         self.alphax_foreground_MI, self.alphay_foreground_MI,
                                                         self.alphax_background_MI, self.alphay_background_MI,
                                                         self.z_split,
                                                         coordinate_type='MULTIPLE_IMAGES',
                                                      x_image=self.x_image, y_image=self.y_image)
    ...
    def test_setup_lens_model(self):
        ...
        npt.assert_equal(len(lens_model_free.lens_model_list), len(self.index_lens_split))
    ...
    def test_point_deflection_model(self):
        ...
        npt.assert_almost_equal(self.kwargs_multiplane_model_point['kwargs_multiplane_model']['x0_interp'](self.x_point, self.y_point), x0_true)
        npt.assert_almost_equal(self.kwargs_multiplane_model_point['kwargs_multiplane_model']['y0_interp'](self.x_point, self.y_point), y0_true)
        ...
        for k in range(0, 2):
            _alpha_x_macromodel, _alpha_y_macromodel = func_list[k]. \
                derivatives(theta_x, theta_y, **self.kwargs_lens_true[k])
        ...
        npt.assert_almost_equal(self.alphax_foreground_point, alphax_fore_true + alpha_x_macro)
        npt.assert_almost_equal(self.alphay_foreground_point, alphay_fore_true + alpha_y_macro)
        npt.assert_almost_equal(
            self.kwargs_multiplane_model_point['kwargs_multiplane_model']['alpha_x_interp_foreground'](self.x_point, self.y_point),
            alphax_fore_true + alpha_x_macro)
        npt.assert_almost_equal(
            self.kwargs_multiplane_model_point['kwargs_multiplane_model']['alpha_y_interp_foreground'](self.x_point, self.y_point),
            alphay_fore_true + alpha_y_macro)

        # ray propagation with the small angle approx
        alpha_x = self.alphax_foreground_point - alpha_x_macro + self.alphax_background_point
        alpha_y = self.alphay_foreground_point - alpha_y_macro + self.alphay_background_point
        x_source_point = self.x0_point + alpha_x * self.Tds
        y_source_point = self.y0_point + alpha_y * self.Tds

        npt.assert_almost_equal(
            self.kwargs_multiplane_model_point['kwargs_multiplane_model']['alpha_x_interp_background'](self.x_point,
                                                                                                       self.y_point),
            self.alphax_background_point)
        npt.assert_almost_equal(
            self.kwargs_multiplane_model_point['kwargs_multiplane_model']['alpha_y_interp_background'](self.x_point,
                                                                                                       self.y_point),
        ...
        beta_x_new, beta_y_new = lens_model_decoupled.ray_shooting(self.x_image[1],
                                                                   self.y_image[1],
                                                                   self.kwargs_lens_free)
        beta_x_true, beta_y_true = self.lens_model_true.ray_shooting(self.x_image[1],
                                                                     self.y_image[1],
                                                                     self.kwargs_lens_true)
    ...
    def test_grid_deflection_model(self):
        ...
        # the full deflection field at the main lens plane is alpha_x_true = alpha_x_foreground - alpha_x_macro
        # therefore alpha_x_foreground = alpha_x_true + alpha_x_macro
        npt.assert_almost_equal(self.alphax_foreground_grid, alphax_fore_true + alpha_x_macro)
        npt.assert_almost_equal(self.alphay_foreground_grid, alphay_fore_true + alpha_y_macro)

        # ray propagation with the small angle approx
        alpha_x = self.alphax_foreground_grid - alpha_x_macro + self.alphax_background_grid
        alpha_y = self.alphay_foreground_grid - alpha_y_macro + self.alphay_background_grid
        ...
        beta_x_new, beta_y_new = lens_model_decoupled.ray_shooting(self.x_point,
                                                                   self.y_point,
                                                                   self.kwargs_lens_free)
        beta_x_true, beta_y_true = self.lens_model_true.ray_shooting(self.x_point,
                                                                     self.y_point,
                                                                     self.kwargs_lens_true)
    ...
    def test_multiple_images_deflection_model(self):
        ...
        # the full deflection field at the main lens plane is alpha_x_true = alpha_x_foreground - alpha_x_macro
        # therefore alpha_x_foreground = alpha_x_true + alpha_x_macro
        npt.assert_almost_equal(self.alphax_foreground_MI, alphax_fore_true + alpha_x_macro)
        npt.assert_almost_equal(self.alphay_foreground_MI, alphay_fore_true + alpha_y_macro)
        ...
        beta_x_new, beta_y_new = lens_model_decoupled.ray_shooting(self.x_image,
                                                                   self.y_image,
                                                                   self.kwargs_lens_free)
        beta_x_true, beta_y_true = self.lens_model_true.ray_shooting(self.x_image,
                                                                     self.y_image,
                                                                     self.kwargs_lens_true)
        npt.assert_allclose(beta_x_new, beta_x_true)
        npt.assert_allclose(beta_y_new, beta_y_true)
    ...
    def test_setup_raytracing_lens_models(self):
        ...
        grid_size = 0.05
        grid_resolution = 0.001
        multiplane_lens_model_list, kwargs_multiplane_lens_model_list = setup_raytracing_lensmodels(
            self.x_image, self.y_image, self.lens_model_true, self.kwargs_lens_true, self.index_lens_split,
            grid_size, grid_resolution
        )
        x_grid, y_grid, _, _ = setup_grids(grid_size, 0.0025)
        lens_model_decoupled_multiple_images = LensModel(**self.kwargs_multiplane_model_MI)

        for i in range(0, len(self.x_image)):
            beta_x, beta_y = multiplane_lens_model_list[i].ray_shooting(x_grid + self.x_image[i],
                                                                        y_grid + self.y_image[i],
                                                                        self.kwargs_lens_free)
            beta_x_true, beta_y_true = self.lens_model_true.ray_shooting(x_grid + self.x_image[i],
                                                                        y_grid + self.y_image[i],
                                                                        self.kwargs_lens_true)
            npt.assert_allclose(beta_x, beta_x_true, 5)
            npt.assert_allclose(beta_y, beta_y_true, 5)
            beta_x, beta_y = multiplane_lens_model_list[i].ray_shooting(self.x_image[i],
                                                                        self.y_image[i],
                                                                        self.kwargs_lens_free)
            beta_x_true, beta_y_true = lens_model_decoupled_multiple_images.ray_shooting(x_grid + self.x_image[i],
                                                                         y_grid + self.y_image[i],
                                                                         self.kwargs_lens_free)
