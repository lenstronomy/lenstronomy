...
def apply_coordinate_transformation(
    ...
    This reproduces the coordinate transformation implemented in the multi-band offset
    feature.
    """
...
def test_transformation():
    ...
    new_ra_center = ra_0_new + M_new[0, 0] * cx + M_new[0, 1] * cy

    new_dec_center = dec_0_new + M_new[1, 0] * cx + M_new[1, 1] * cy
...
def test_translation_only():
    ...
    ra_center_new = ra_0_new + M_new[0, 0] * cx + M_new[0, 1] * cy

    dec_center_new = dec_0_new + M_new[1, 0] * cx + M_new[1, 1] * cy
...
def test_rotation_only():
    ...
    # Original center
    ra_center_old = ra_0_old + M_old[0, 0] * cx + M_old[0, 1] * cy

    dec_center_old = dec_0_old + M_old[1, 0] * cx + M_old[1, 1] * cy
...
def test_apply_restore_multiband_offsets():
    ...
    image_model_list = likelihood.imSim._imageModel_list
    data_band_1 = image_model_list[1].Data

    original_ra = data_band_1._ra_at_xy_0
    original_dec = data_band_1._dec_at_xy_0
    original_M = np.copy(data_band_1._Mpix2a)
    ...
    # Apply
    # --------------------------------------------------------
    backup = likelihood._apply_multiband_offsets(kwargs_special)
    ...
    print("PASS: apply_multiband_offsets modifies coordinates")

    # --------------------------------------------------------
    # Restore
    # --------------------------------------------------------

    likelihood._restore_multiband_offsets(backup)
    ...
    print("PASS: restore_multiband_offsets restores coordinates")
...
def test_multiband_offsets_change_image_and_restore():
    ...
    backup = likelihood._apply_multiband_offsets(kwargs_special_true)
    ...
    print("PASS: multiband offsets change image and restore correctly")
...
def test_repeated_apply_gives_same_image():
    ...
    print("PASS: repeated apply gives identical result")
