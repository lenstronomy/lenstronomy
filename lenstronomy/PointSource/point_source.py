...
class PointSource(object):
    ...
    def update_search_window(
        ...
        :param search_window: search_window: window size of the image
            position search with the lens equation solver.
        :param x_center: center of search window
        :param y_center: center of search window
        :param min_distance: minimum search distance
        :param only_from_unspecified: bool, if True, only sets keywords
            that previously have not been set
    ...
    def set_save_cache(self, save_cache):
        ...
        :param save_cache: bool, if True, saves (or uses a previously
            saved) values
        :return: updated class and subclass instances to either save or
            not save the point source information in cache
        """
        self._set_save_cache(save_cache)
        self._save_cache = save_cache

    def _set_save_cache(self, save_cache):
        """Set the save cache boolean to new value. This function is for use
        within this class for temporarily set the cache within a single
        routine.
    ...
    def image_position(
        ...
        :param k: None, int or boolean list; only returns a subset of
            the model predictions
        :param original_position: boolean (only applies to
            'LENSED_POSITION' models), returns the image positions in
            the model parameters and does not re-compute images (which
            might be differently ordered) in case of the lens equation
            solver
        :param additional_images: if True, solves the lens equation for
            additional images
        :type additional_images: bool
        :param width_search_factor: searches on wider area than default
            setting (usually tided to image frame)
        :type width_search_factor: int of float >1
        :return: list of: list of image positions per point source model
            component
        """
        x_image_list = []
        y_image_list = []
    ...
    def point_source_list(self, kwargs_ps, kwargs_lens, k=None, with_amp=True):
        """Returns the coordinates and amplitudes of all point sources in a
        single array.

        :param kwargs_ps: point source keyword argument list
        :param kwargs_lens: lens model keyword argument list
        :param k: None, int or list of int's to select a subset of the
            point source models in the return
        :param with_amp: bool, if False, ignores the amplitude
            parameters in the return and instead provides ones for each
            point source image
    ...
    def check_image_positions(self, kwargs_ps, kwargs_lens, tolerance=0.001):
        """Checks whether the point sources in kwargs_ps satisfy the lens
        equation with a tolerance (computed by ray-tracing in the source plane)

        :param kwargs_ps: point source keyword argument list
        :param kwargs_lens: lens model keyword argument list
        :param tolerance: Euclidian distance between the source
            positions ray-traced backwards to be tolerated
        :return: bool: True, if requirement on tolerance is fulfilled,
            False if not.
        """
        x_image_list, y_image_list = self.image_position(kwargs_ps, kwargs_lens)
        for i, model in enumerate(self.point_source_type_list):
    ...
    def set_amplitudes(self, amp_list, kwargs_ps):
        """Translates the amplitude parameters into the convention of the
        keyword argument list currently only used in SimAPI to transform
        magnitudes to amplitudes in the lenstronomy conventions.

        :param amp_list: list of model amplitudes for each point source
            model. This list should include all of the point source
            models even if flux_from_point_source is False for any of
            them. In that case, the amplitudes will not be changed for
            those models.
    ...
    def check_positive_flux(cls, kwargs_ps):
        ...
        :param kwargs_ps: point source keyword argument list
        :return: bool, True, if all 'point_amp' parameters are positive
            semi-definite
        """
        pos_bool = True
...
def _sort_position_by_original(x_o, y_o, x_solved, y_solved):
    ...
    :param x_solved: numpy array; solved image positions with
        potentially more or fewer images
    :param y_solved: numpy array; solved image positions with
        potentially more or fewer images
    :return: sorted new image positions with the order best matching the
        original positions first, and then all other images in the same
        order as solved for
    """
