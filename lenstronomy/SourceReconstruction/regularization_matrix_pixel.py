"""
This module provides functions to construct regularization matrices for pixelated source
plane reconstructions in gravitational lensing. These matrices are used to constrain
the pixel amplitudes in linear inversion problems, promoting desired properties such
as smoothness.
...
def _gradient_regularization_matrix_pixel(xlen, ylen):
    ...
    for i_y in range(ylen - 1):
        Umatrix[xlen*(i_y+1) : xlen*(i_y+2), xlen*i_y : xlen*(i_y+1)] = block1
        Umatrix[xlen*i_y : xlen*(i_y+1), xlen*(i_y+1) : xlen*(i_y+2)] = block1
...
def _curvature_regularization_matrix_pixel(xlen, ylen):
    ...
    for i_y in range(ylen - 2):
        Umatrix[xlen*(i_y+2) : xlen*(i_y+3), xlen*i_y : xlen*(i_y+1)] = block2
        Umatrix[xlen*i_y : xlen*(i_y+1), xlen*(i_y+2) : xlen*(i_y+3)] = block2
