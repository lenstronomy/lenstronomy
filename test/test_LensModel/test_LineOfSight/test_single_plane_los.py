...
class TestSinglePlaneLOS(object):
    ...
    addition of LOS and LOS_MINIMAL as profiles. With all params in
    self.kwargs_los set to zero, the results should be the same as the
    non-LOS cases originally tested. The test_los_vs_multiplane checks
    that a multiplane setup with three shear planes returns the same as
    the LOS and LOS MINIMAL models.
    """
    ...
    def test_los_versus_multiplane(self):
        """This function asserts that the outcome from LOS and LOS MINIMAL is
        the same as MultiPlane."""
