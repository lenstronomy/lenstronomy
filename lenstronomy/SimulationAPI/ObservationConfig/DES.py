...
class DES(object):
    ...
    def __init__(self, band="g", psf_type="GAUSSIAN", coadd_years=3):
        ...
        if band == "g":
            ...
            raise ValueError(
                "band %s not supported! Choose 'g', 'r', 'i', 'z', or 'Y'." % band
            )
        ...
        if coadd_years > 6 or coadd_years < 1:
            raise ValueError(
                " %s coadd_years not supported! Choose an integer between 1 and 6."
                % coadd_years
            )
        elif coadd_years != 3:
            self.obs["num_exposures"] = (coadd_years * 10) // 3

        self.camera = {"read_noise": 7, "pixel_scale": 0.263, "ccd_gain": 4}
