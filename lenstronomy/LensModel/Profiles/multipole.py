    ...
    def function(self, x, y, m, a_m, phi_m, q, center_x=0, center_y=0):
        ...
        if q==1:
            ...
            if m==3:
                ...
                phirot = param_util.cart2polar(-r*np.sin(phi), r*np.cos(phi))[1]-np.pi/2 #find angle corresponding to phi in (-3pi/2, pi/2]
                phirotsymm = param_util.cart2polar(r*np.sin(phi), r*np.cos(phi))[1]-np.pi/2 #find angle corresponding to -phi in (-3pi/2, pi/2]
                f_ = a_m*np.sqrt(q)*r*((F_m3_1(phi, q)-F_m3_1(phisymm, q))/2 *np.cos(m*phi_m) + (
                    F_m3_2(phirot, q)-F_m3_2(phirotsymm, q))/2 *np.sin(m*phi_m))
            
            elif m==4:
                f_ = a_m*np.sqrt(q)*r*(F_m4_1(phi, q=q)*np.cos(m*phi_m) + F_m4_2(phi, q=q)*np.sin(m*phi_m))
    
            else:
                raise ValueError('Implementation of multipoles perturbation with m>4 for general axis ratio q not available.')
    ...
    def derivatives(self, x, y, m, a_m, phi_m, q, center_x=0, center_y=0):
        ...
        if q==1:
            ...
            if m==3:
                ...
                phirot = param_util.cart2polar(-r*np.sin(phi), r*np.cos(phi))[1]-np.pi/2 #find angle corresponding to phi in (-3pi/2, pi/2]
                phirotsymm = param_util.cart2polar(r*np.sin(phi), r*np.cos(phi))[1]-np.pi/2 #find angle corresponding to -phi in (-3pi/2, pi/2]
                alpha_x_1_pos, alpha_y_1_pos = deflection_m3_base(r, phi, q, a_3=a_m, phi_3=0)
                alpha_x_1_neg, alpha_y_1_neg = deflection_m3_base(r, phisymm, q, a_3=a_m, phi_3=0)
                alpha_x_2_pos, alpha_y_2_pos = deflection_m3_base(r, phirot, q, a_3=a_m, phi_3=np.pi/6)
                alpha_x_2_neg, alpha_y_2_neg = deflection_m3_base(r, phirotsymm, q, a_3=a_m, phi_3=np.pi/6)
                f_x = np.cos(m*phi_m)*(alpha_x_1_pos+alpha_x_1_neg)/2 + np.sin(m*phi_m)*(alpha_x_2_pos-alpha_x_2_neg)/2
                f_y = np.cos(m*phi_m)*(alpha_y_1_pos-alpha_y_1_neg)/2 + np.sin(m*phi_m)*(alpha_y_2_pos+alpha_y_2_neg)/2
    
            elif m==4:
                F_m4 = F_m4_1(phi, q=q)*np.cos(m*phi_m) + F_m4_2(phi, q=q)*np.sin(m*phi_m)
                F_m4_prime = F_m4_1_derivative(phi, q=q)*np.cos(m*phi_m) + F_m4_2_derivative(phi, q=q)*np.sin(m*phi_m)
...
def phi_ell(phi,q):
    return phi-np.arctan2(np.sin(phi), np.cos(phi))+np.arctan2(np.sin(phi), q*np.cos(phi))

def F_m3_1(phi,q): 
    term1 = np.cos(phi) * (q*(3+q**2) * np.log(1+q**2+(q**2-1)*np.cos(2*phi)) - (np.log(2)*(1+q)**2 - 2*(1-q)*(1+q)**2*(1+np.log(2)/4) + (1-q**2)**2/4))
    term2 = 2 * np.sin(phi) * (q*(q**2+3)*phi - (1+3*q**2)* phi_ell(phi,q)) # Expression valid in (-pi, pi]
...
def F_m3_1_derivative(phi,q):
    term1 = - np.cos(phi) *q*(3+q**2) * 2*(q**2-1)*np.sin(2*phi)/(1+q**2+(q**2-1)*np.cos(2*phi)) + np.sin(phi) * (
        -q*(3+q**2) * np.log(1+q**2+(q**2-1)*np.cos(2*phi)) + np.log(2)*(1+q)**2 - 2*(1-q)*(1+q)**2*(1+np.log(2)/4) + (1-q**2)**2/4)
    term2 = 2 * np.cos(phi) * (q*(q**2+3)*phi - (1+3*q**2)* phi_ell(phi,q)) + 2 * np.sin(phi) * (
            q*(q**2+3) -(1+3*q**2)* q/(q**2*np.cos(phi)**2+np.sin(phi)**2)) # Expression valid in (-pi, pi]
    return (term1+term2)/(2*(1-q**2)**2)
...
def deflection_m3_base(r, phi, q, a_3, phi_3):
    ...
    F_m3_prime = F_m3_1_derivative(phi, q=q)*np.cos(3*phi_3) + F_m3_2_derivative(phi, q=q)*np.sin(3*phi_3)
...
def F_m4_1(phi,q):
    term1 = -4*np.sqrt(2)*(1+4*q**2+q**4+(q**4-1)*np.cos(2*phi))/((3*(1-q**2)**2)*np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi)))
    term2 = (1+6*q**2+q**4)/(1-q**2)**(5/2) * np.cos(phi)* np.arctan((np.sqrt(2*(1-q**2))*np.cos(phi))/np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi)))
    term3 = (1+6*q**2+q**4)/(1-q**2)**(5/2) * np.sin(phi)* np.log(np.sqrt(1-q**2)*np.sin(phi)/q + np.sqrt(1+(1-q**2)/q**2*np.sin(phi)**2))
    
    return term1+term2+term3

def F_m4_1_derivative(phi,q):
    term1 = -4*np.sqrt(2)*(1+q**4+(q**4-1)*np.cos(2*phi))*np.sin(2*phi)/(3*(1-q**2)*(1+q**2+(q**2-1)*np.cos(2*phi))**(3/2))
    term2 = -(1+6*q**2+q**4)/(1-q**2)**(5/2) * (np.sin(phi) * np.arctan((np.sqrt(2*(1-q**2))*np.cos(phi))/np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi)))
                                               + np.sqrt(2*(1-q**2))*np.sin(2*phi)/(2*np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi))))
    term3 = (1+6*q**2+q**4)/(1-q**2)**(5/2) * np.cos(phi) * (np.log(np.sqrt(1-q**2)*np.sin(phi)/q + np.sqrt(1+(1-q**2)/q**2*np.sin(phi)**2))
                                                          + np.sqrt(1-q**2)/q*np.sin(phi)/np.sqrt(1+(1-q**2)/q**2*np.sin(phi)**2))
    
    return term1+term2+term3

def F_m4_2(phi,q):
    term1 = -4*np.sqrt(2)*q/(3*(1-q**2)) * np.sin(2*phi)/np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi))
    term2 = -4*q*(1+q**2)/(1-q**2)**(5/2) * np.sin(phi) * np.arctan((np.sqrt(2*(1-q**2))*np.cos(phi))/np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi)))
    term3 = 4*q*(1+q**2)/(1-q**2)**(5/2) * np.cos(phi) * np.log(np.sqrt(1-q**2)*np.sin(phi)/q + np.sqrt(1+(1-q**2)/q**2*np.sin(phi)**2))
    
    return term1+term2+term3

def F_m4_2_derivative(phi,q):
    term1 = -8*np.sqrt(2)*q/(6*(1-q**2))*(-(1-q**2)*np.sin(2*phi)**2/(1+q**2+(q**2-1)*np.cos(2*phi))**(3/2) + 2*np.cos(2*phi)/np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi)))
    term2 = 4*q*(1+q**2)/(1-q**2)**(5/2) *(-np.cos(phi) * np.arctan((np.sqrt(2*(1-q**2))*np.cos(phi))/np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi)))
                                               + 2*np.sqrt(2*(1-q**2))*np.sin(phi)**2/(2*np.sqrt(1+q**2+(q**2-1)*np.cos(2*phi))))
    term3 =  4*q*(1+q**2)/(1-q**2)**(5/2) *(-np.sin(phi) * np.log(np.sqrt(1-q**2)*np.sin(phi)/q + np.sqrt(1+(1-q**2)/q**2*np.sin(phi)**2))
                                            + np.sqrt(1-q**2)/q*np.cos(phi)**2/np.sqrt(1+(1-q**2)/q**2*np.sin(phi)**2))
