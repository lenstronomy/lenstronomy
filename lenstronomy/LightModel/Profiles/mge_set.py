...
class MGESet(object):
    ...
    def _sigma(self, sigma_min, sigma_max):
        """
        ...
        """
        sigmas = np.logspace(start=np.log10(sigma_min), stop=np.log10(sigma_max), num=self._ncomp, endpoint=True)
        return sigmas

    def function(self, x, y, amp, sigma_min, sigma_max, center_x=0, center_y=0):
        ...
        """
        sigma = self._sigma(sigma_min=sigma_min, sigma_max=sigma_max)
        f_ = np.zeros_like(x, dtype=float)
        for i in range(len(amp)):
            f_ += self.gaussian.function(x, y, amp[i], sigma[i], center_x, center_y)
        return f_

    def total_flux(self, amp, sigma_min, sigma_max, center_x=0, center_y=0):
    ...
    def function_split(self, x, y, amp, sigma_min, sigma_max, center_x=0, center_y=0):
    ...
    def light_3d(self, r, amp, sigma_min, sigma_max):
