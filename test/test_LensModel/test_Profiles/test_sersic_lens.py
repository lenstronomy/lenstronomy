...
class TestSersic(object):
    ...
    def test_derivatives(self):
        ...
        f_x2, f_y2 = self.sersic_2.derivatives(
            x, y, n_sersic, R_sersic, k_eff, 0, 0.00000001
        )
        ...
        values2 = self.sersic_2.derivatives(
            x, y, n_sersic, R_sersic, k_eff, 0, 0.00000001
        )
        ...
        values2 = self.sersic_2.derivatives(
            0.3, -0.2, n_sersic, R_sersic, k_eff, 0, 0.00000001
        )
        values = self.sersic.derivatives(
            0.3, -0.2, n_sersic, R_sersic, k_eff, 0, 0.00000001
        )
    ...
    def test_differentails(self):
        ...
        f_xx_ = d_alpha_dr * calc_util.d_r_dx(
            x_, y_
        ) * x_ / r + alpha * calc_util.d_x_diffr_dx(x_, y_)
        f_yy_ = d_alpha_dr * calc_util.d_r_dy(
            x_, y_
        ) * y_ / r + alpha * calc_util.d_y_diffr_dy(x_, y_)
        f_xy_ = d_alpha_dr * calc_util.d_r_dy(
            x_, y_
        ) * x_ / r + alpha * calc_util.d_x_diffr_dy(x_, y_)
    ...
    def test_convergernce(self):
        ...
        kappa = (f_xx + f_yy) / 2.0
        assert kappa[0] > 0
        flux = self.sersic_light.function(
            x, y, amp=1.0, R_sersic=R_sersic, n_sersic=n_sersic
        )
        flux /= flux[0]
        kappa /= kappa[0]
        npt.assert_almost_equal(flux[1], kappa[1], decimal=5)

        xvalues = np.linspace(0.5, 3.0, 100)

        e1, e2 = 0.4, 0.0
        q = ellipticity2phi_q(e1, e2)[1]
        kappa_ellipse = self.sersic_2.projected_mass(
            xvalues, 0, q, n_sersic, R_sersic, k_eff
        )
        fxx, _, _, fyy = self.sersic_2.hessian(
            xvalues, 0, n_sersic, R_sersic, k_eff, e1, e2
        )
