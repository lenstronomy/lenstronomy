...
class BPLMajorAxis(LensProfileBase):
    ...
    def function(self, x, y, b, a, a_c, r_c, q):
        ...
        vectorized_quad = np.vectorize(
            lambda aa, bb: quad(
                self.integrand_psi, 0, 1, args=(aa, bb, b, a, a_c, r_c, q)
            )[0]
        )
    ...
    def derivatives(self, x, y, b, a, a_c, r_c, q):
        ...
        alpha1conj = (
            ...
            / Z
            * (b / R_el) ** (a - 1)
            * hyp2f1(1 / 2, (3 - a) / 2, (5 - a) / 2, R_el**2 * zeta2)
        )
        ...
        alpha2conj = (
            ...
            * (b / r_c) ** (a - 1)
            * (
                2 / (3 - a_c) * self.F((3 - a_c) / 2, C)
                - 2 / (3 - a) * self.F((3 - a) / 2, C)
                - S0_arr
            )
        )
    ...
    def hessian(self, x, y, b, a, a_c, r_c, q):
        ...
        alpha1 = (
            ...
            / Z
            * (b / R_el) ** (a - 1)
            * hyp2f1(1 / 2, (3 - a) / 2, (5 - a) / 2, zeta2 * (R_el) ** 2)
        )
        ...
        gamma2conj = 2 * (r_c**2 / Z**2) * (3 - a) / self.Beta_func(a) * (
            b / r_c
        ) ** (a - 1) * (
    ...
    def F(self, a, z):
        if a == 0.5:
            ...
            return (1 / (1 - 2 * a)) * (
                hyp2f1(a, 1, a + 1, z) - 2 * a * hyp2f1(1 / 2, 1, 3 / 2, z)
            )
        ...
        if isinstance(R_el, int) or isinstance(R_el, float):
            ...
            if R_el < r_c:
                result = (
                    ...
                    * z_el
                    * (
                        hyp2f1(a / 2, 1, 3 / 2, z_el**2)
                        - hyp2f1(a_c / 2, 1, 3 / 2, z_el**2)
                    )
                )
                return result  # 返回计算
            else:
            ...
            result[R_el < r_c] = (
                ...
                * z_el_array
                * (
                    hyp2f1(a / 2, 1, 3 / 2, z_el_array**2)
                    - hyp2f1(a_c / 2, 1, 3 / 2, z_el_array**2)
                )
            )
            result[R_el >= r_c] = 0.0
        return result
    ...
    def kappa_mean(self, R, alpha, alpha_c, b, r_c):
        ...
        if R < r_c:
            ...
            temp3 = (
                ...
                * z**3
                * (
                    hyp2f1(alpha / 2, 1, 5 / 2, z**2)
                    - hyp2f1(alpha_c / 2, 1, 5 / 2, z**2)
                )
            )
            temp4 = (
                -2
                ...
                * 1**3
                * (
                    hyp2f1(alpha / 2, 1, 5 / 2, 1**2)
                    - hyp2f1(alpha_c / 2, 1, 5 / 2, 1**2)
                )
            )
