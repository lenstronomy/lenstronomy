...
class LightProfileAnalysis(object):
    ...
    def ellipticity(
        ...
        """Make sure that the window covers all the light, otherwise the
        moments may give a too low answers.

        :param kwargs_light: keyword argument list of profiles
        :param center_x: center of profile, if None takes it from the
            first profile in kwargs_light
        :param center_y: center of profile, if None takes it from the
            first profile in kwargs_light
        :param model_bool_list: list of booleans to select subsets of
            the profile
        :param grid_spacing: grid spacing over which the moments are
            computed
        :param grid_num: grid size over which the moments are computed
        :param iterative: if True iteratively adopts an eccentric mask
            to overcome edge effects
    ...
    def half_light_radius(
        ...
        """Computes numerically the half-light-radius of the deflector light
        and the total photon flux.
    ...
    def multi_gaussian_decomposition(
        ...
        """Multi-gaussian decomposition of the lens light profile (in
        1-dimension)
        ...
        :param model_bool_list: list of booleans to select subsets of
            the profile
        :param grid_spacing: grid spacing over which the moments are
            computed for the half-light radius
        :param grid_num: grid size over which the moments are computed
        :param n_comp: maximum number of Gaussian's in the MGE
        :param r_h: float, half light radius to be used for MGE
            (optional, otherwise using a numerical grid)
