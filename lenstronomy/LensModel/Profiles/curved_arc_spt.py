...
class CurvedArcSPT(LensProfileBase):
    """Curved arc model based on SIS+MST with an additional non-linear shear
    distortions applied on the source coordinates around the center.

    This profile is effectively a Source Position Transform of a curved
    arc and a shear distortion.
    """
    ...
    def derivatives(
        ...
        f_x_c, f_y_c = self._curve.derivatives(
            x,
            y,
            tangential_stretch,
            radial_stretch,
        ...
        f_x_b, f_y_b = self._distort.derivatives(
            beta_x, beta_y, gamma1, gamma2, ra_0=center_x, dec_0=center_y
        )
