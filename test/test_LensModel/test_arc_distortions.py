...
class TestArcDistortions(object):
    ...
    def test_curved_arc_estimate_tan_diff(self):
        ...
        kwargs_arc = ext.curved_arc_estimate(x_0, y_0, kwargs_lens, tan_diff=True, smoothing_3rd=0.01)
    ...
    def test_analytic_differentials_spp(self):
        ...
        x = np.append(np.linspace(start=0.1, stop=theta_E-0.1, num=9), np.linspace(start=theta_E + 0.1, stop=3 * theta_E, num=9))
        y = np.zeros_like(x)
        ...
        def _lambda_t_dr_analytic(r, theta_E, gamma):
            ...
            :return:
            """
            return (1 - gamma) * (theta_E / r) ** gamma / (theta_E * (1 - (theta_E/r)**(gamma - 1))**2)
        ...
        def _lambda_r_dr_analytic(r, theta_E, gamma):
            ...
            :return:
            """
            return (1 - gamma) * (2 - gamma) * (theta_E / r) ** gamma / (theta_E * (1 + (theta_E/r)**(gamma - 1) * (gamma - 2))**2)

        dlambda_r_drad_analytic = _lambda_r_dr_analytic(x, theta_E, gamma)
        npt.assert_almost_equal(dlambda_r_drad_analytic / dlambda_rad_drad, 1, decimal=2)
    ...
    def test_analytic_differential_pemd(self):
        ...
        ext = LensModelExtensions(lensModel=lens_model_class)
        # we leave out the very high magnification region where the numerical differentials might not be as accurate as required in the tests
        x = np.append(np.linspace(start=0.2 * theta_E, stop=theta_E * (1-0.2), num=9),
        ...
        def _dlambda_t_dt_analytic(x, y, theta_E, gamma, q, phi_G):
            ...
            dr_de_t = epsilon * np.sin(2 * (phi - phi_G)) / np.sqrt(1 - epsilon * np.cos(2 * (phi - phi_G)))
        ...
        lambda_rad, lambda_tan, orientation_angle, dlambda_tan_dtan, dlambda_tan_drad, dlambda_rad_drad, dlambda_rad_dtan, dphi_tan_dtan, dphi_tan_drad, dphi_rad_drad, dphi_rad_dtan = ext.radial_tangential_differentials(x, y, kwargs_lens)
        dlambda_tan_dtan_analytic, dlambda_t_dr_analtic = _dlambda_t_dt_analytic(x, y, theta_E, gamma, q, phi_G)
        ...
        npt.assert_almost_equal(dlambda_tan_dtan_analytic / dlambda_tan_dtan, 1, decimal=2)
