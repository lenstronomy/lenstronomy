...
def param_transform(x, y, theta_E, gamma, e1, e2, center_x=0.0, center_y=0.0):
    """Converts the parameters from lenstronomy definitions (as defined in
    PEMD) to the definitions of Tessore+ (2015)"""
    t = gamma - 1
...
def alpha(x, y, b, q, t, Omega=None):
    ...
    :param x: x-coordinate (angle)
    :param y: y-coordinate (angle)
    :param b: Einstein radius (angle), pay attention to specific
        definition!
    :param q: axis ratio
...
@jit(
    fastmath=True
)  # Because of the reduction nature of this, relaxing commutativity actually matters a lot (4x speedup).
