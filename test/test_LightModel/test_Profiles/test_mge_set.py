...
class TestMGESet(object):
    ...
    def test_function(self):
        ...
        flux_mg = self.multi_gauss.function(x, y, amp=amp, sigma=sigma, center_x=0, center_y=0)
        flux_mge = self.mge_set.function(x, y, amp=amp, sigma_min=sigma_min, sigma_max=sigma_max,
    ...
    def test_function_split(self):
        ...
        sigma = np.logspace(start=np.log10(sigma_min), stop=np.log10(sigma_max), num=self.n_comp)
        amp = np.ones(self.n_comp)
        flux_mg = self.multi_gauss.function_split(x, y, amp=amp, sigma=sigma, center_x=0, center_y=0)
        flux_mge = self.mge_set.function_split(x, y, amp=amp, sigma_min=sigma_min, sigma_max=sigma_max,
                                         center_x=0, center_y=0)
    ...
    def test_light3d(self):
        ...
        light_3d_mge = self.mge_set.light_3d(r, amp=amp, sigma_min=sigma_min, sigma_max=sigma_max)
