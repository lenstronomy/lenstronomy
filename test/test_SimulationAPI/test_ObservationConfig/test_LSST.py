...
class TestLSST(unittest.TestCase):
    ...
    def test_LSST_obs(self):
        ...
        for config, setting in self.obs_settings.items():
            self.assertEqual(self.u.obs[config], getattr(self.u_band, setting), msg=f"{config} did not match")
            ...
            self.assertEqual(self.z.obs[config], getattr(self.z_band, setting), msg=f"{config} did not match")
            self.assertEqual(self.y.obs[config], getattr(self.y_band, setting), msg=f"{config} did not match")
