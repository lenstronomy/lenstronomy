...
def potential_from_kappa_grid(kappa, grid_spacing):
    """Lensing potential :math:`\\psi ({\\vec {\\theta }})` on the convergence
    grid :math:`\\kappa`.
...
def potential_from_kappa_grid_adaptive(
    ...
    """Lensing potential on the convergence grid the computation is performed
    as a convolution of the Green's function with the convergence map using
    FFT.

    :param kappa_high_res: 2d grid of convergence values
    :param grid_spacing: scale of an individual pixel (per axis) of grid
    :param low_res_factor: lower resolution factor of larger scale
        kernel.
    :param high_res_kernel_size: int, size of high resolution kernel in
        units of degraded pixels
    :return: lensing potential in a 2d grid at positions x_grid, y_grid
    """
...
def deflection_from_kappa_grid_adaptive(
    ...
    """Deflection angles on the convergence grid with adaptive FFT the
    computation is performed as a convolution of the Green's function with the
    convergence map using FFT The grid is returned in the lower resolution
    grid.
...
def potential_kernel(num_pix, delta_pix):
    """Numerical gridded integration kernel for convergence to lensing kernel
    with given pixel size.
...
def deflection_kernel(num_pix, delta_pix):
    """Numerical gridded integration kernel for convergence to deflection angle
    with given pixel size.

    :param num_pix: integer; number of pixels of kernel per axis, should
        be odd number to have a defined center
    :param delta_pix: pixel size (per dimension in units of angle)
    :return: kernel for x-direction and kernel of y-direction deflection
        angles
    """
    x_shift, y_shift = util.make_grid(num_pix=num_pix, delta_pix=delta_pix)
    r2 = x_shift**2 + y_shift**2
