...
from lenstronomy.LensModel.Microlensing.adaptive_boundary_mesh import adaptive_boundary_mesh
...
class Test_adaptive_boundary_mesh:
    ...
    def test_within_distance(self):
        ...
        distances_within = within_distance(center_points_within[:, 0], center_points_within[:, 1], test_point, delta_beta, beta_s)
        assert np.all(distances_within)

        # Test points not within the threshold
        distances_not_within = within_distance(center_points_not_within[:, 0], center_points_not_within[:, 1], test_point, delta_beta, beta_s)
    ...
    def test_ABM(self):
        ...
        final_eta = 3.23
        kwargs_lens = [{'theta_E': 0.0001, 'center_x': 0.000025, 'center_y': 0.00001666666}]

        side_length, total_number_of_rays_shot , image_subset_centers_x, image_subset_centers_y = adaptive_boundary_mesh(source_position, L, beta_0, beta_s, n_p, eta, number_of_iterations, final_eta, kwargs_lens)
