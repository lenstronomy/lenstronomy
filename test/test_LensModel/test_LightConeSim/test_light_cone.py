...
class TestLightCone(object):
    ...
    def setup(self):
        ...
        logM_200_list = [8, 9, 10]
        c_list = [20, 10, 8]
        kwargs_lens = []
        kwargs_lens_interp = []
        grid_spacing = 0.01  # in units arc seconds
        x_grid, y_grid = util.make_grid(numPix=500, deltapix=grid_spacing)
        ...
        for i, z in enumerate(redshift_list):
            ...
            alpha_Rs, Rs = lens_cosmo.nfw_physical2angle(M=10**(logM_200_list[i]), c=c_list[i])
            kwargs_nfw = {'Rs': Rs, 'alpha_Rs': alpha_Rs, 'center_x': 0, 'center_y': 0}
            kwargs_lens.append(kwargs_nfw)
            kappa_map = single_plane.kappa(x_grid, y_grid, [kwargs_nfw])
            kappa_map = util.array2image(kappa_map)
            mass_map = lens_cosmo.epsilon_crit_angle * kappa_map * grid_spacing ** 2
            mass_map_list.append(mass_map)
            npt.assert_almost_equal(np.log10(np.sum(mass_map)), logM_200_list[i], decimal=0)
            grid_spacing_mpc = lens_cosmo.arcsec2phys_lens(grid_spacing)
            grid_spacing_list_mpc.append(grid_spacing_mpc)
            f_x, f_y = convergence_integrals.deflection_from_kappa_grid(kappa_map, grid_spacing)
            f_ = convergence_integrals.potential_from_kappa_grid(kappa_map, grid_spacing)
        ...
        self.lightCone = LightCone(mass_map_list, grid_spacing_list_mpc, redshift_list)
