...
def potential_from_kappa(kappa, deltaPix):
    """
    ...
    """
    num_pix = len(kappa) * 2
    kernel = potential_kernel(num_pix, deltaPix)
    f_ = scp.fftconvolve(kappa, kernel, mode='same') / np.pi * deltaPix ** 2
    return f_


def deflection_from_kappa(kappa, deltaPix):
    """
    ...
    """
    num_pix = len(kappa) * 2
    kernel_x, kernel_y = deflection_kernel(num_pix, deltaPix)
    f_x = scp.fftconvolve(kappa, kernel_x, mode='same') / np.pi * deltaPix ** 2
    f_y = scp.fftconvolve(kappa, kernel_y, mode='same') / np.pi * deltaPix ** 2
    return f_x, f_y
