...
class PixelatedSourceReconstruction(object):
    ...
    It handles initialization of data, lens model, and PSF kernel, and provides methods
    for generating the M and b matrices for source reconstruction based on different
    likelihood methods.
    """
<<<<<<< HEAD
    ...
    def __init__(self, data_class, psf_class, lens_model_class, source_pixel_grid_class, kwargs_lens, verbose=False):
        ...
        if (transform_pix2angle_source[0,1] != 0 or transform_pix2angle_source[1,0] != 0 or 
            transform_pix2angle_source[0,0] != transform_pix2angle_source[1,1]):
=======

    def __init__(
        self, kwargs_data, lens_model_list, kwargs_lens, kernel, verbose=False
    ):
        """
        ...
        self._deltaPix = kwargs_data["transform_pix2angle"][
            0, 0
        ...
        for check_dim in range(2):
            if (
                shape_kernel_cut[check_dim] % 2 == 0
                or shape_kernel_cut[check_dim] < 2 * self._numPix - 1
            ):
                raise ValueError(
                    "PSF kernel size must be odd and at least (2 * numPix - 1) "
        ...
        self._beta_x_grid, self._beta_y_grid = lens_model_class.ray_shooting(
            self._x_grid, self._y_grid, kwargs=kwargs_lens
        )

        self._primary_beam = kwargs_data.get(
            "antenna_primary_beam", None
        )  # Optional primary beam map
        self._logL_method = kwargs_data.get(
            "likelihood_method", "diagonal"
        )  # Likelihood method for M and b generation
        self._verbose = verbose
...
<<<<<<< HEAD
        ...
        if data_class._logL_method == "interferometry_natwt":
            for check_dim in range(2):
...
=======
    ...
    def generate_M_b(self, x_min, x_max, y_min, y_max):
        """Generates the M and b matrices for source reconstruction based on the
        selected likelihood method.
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
...
=======
        ...
        for coord, name in zip(
            ...
            if not isinstance(coord, int):
                raise TypeError(
                    f"'{name}' must be an integer, but got {type(coord).__name__}."
                )

        if not (
            0 <= x_min < x_max <= self._numPix and 0 <= y_min < y_max <= self._numPix
        ):
            raise ValueError(
        ...
        for i in tqdm(range(N_lensed), desc="Running (iteration times vary)"):
            ...
            for j in range(N_lensed):
                if j < i:
                    ...
                    M[i, j] = np.sum(
                        lensed_pixel_conv_set[i] * lensed_pixel_conv_set[j]
                    )
...
=======
    ...
    def lens_pixel_source_of_a_rectangular_region(self, x_min, x_max, y_min, y_max):
        ...
        region, considering lensing deflections and applying bilinear interpolation.
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
...
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
        ...
        x_ceiling = x_floor + 1
        y_floor = np.floor((beta_y_grid_2d - self._source_min_y) / self._pixel_width_source).astype(int)
        y_ceiling = y_floor + 1

        # Calculate fractional pixel offsets for bilinear interpolation
<<<<<<< HEAD
        delta_x_pixel = (beta_x_grid_2d - self._source_min_x) / self._pixel_width_source - x_floor
        delta_y_pixel = (beta_y_grid_2d - self._source_min_y) / self._pixel_width_source - y_floor
...
<<<<<<< HEAD
                if (x_ceiling[i][j] < 0 or x_floor[i][j] > (self._nx_source - 1) or 
                    y_ceiling[i][j] < 0 or y_floor[i][j] > (self._ny_source - 1)):
=======
                if (
                    x_ceiling[i][j] < x_min
                    or x_floor[i][j] > (x_max - 1)
                    or y_ceiling[i][j] < y_min
                    or y_floor[i][j] > (y_max - 1)
                ):
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
                    continue
...
=======
                source_pixel_index00 = (
                    xlen * (y_floor[i][j] - y_min) + x_floor[i][j] - x_min
                )
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
...
=======
        ...
        if nx != self._numPix or ny != self._numPix:
            raise ValueError(
                f"Input image size ({nx}, {ny}) must match the defined "
...
<<<<<<< HEAD
                n_beta = i * self._numPix + j # Assuming beta_x_grid and beta_y_grid are flattened row-major
=======
                n_beta = (
                    i * ny + j
...
<<<<<<< HEAD
                n_x = np.floor((cor_beta_x - self._source_min_x) / self._pixel_width_source).astype(int)
                n_y = np.floor((cor_beta_y - self._source_min_y) / self._pixel_width_source).astype(int)
...
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
                ...
                if n_x > self._nx_source - 1 or n_y > self._ny_source - 1 or n_x < 0 or n_y < 0:
                    lensed_image[i, j] = 0
                else:
...
<<<<<<< HEAD
                    ...
                    weight_upper_left = np.abs(self._source_min_y + n_y*self._pixel_width_source + self._pixel_width_source - cor_beta_y) * (
                        np.abs(self._source_min_x + n_x*self._pixel_width_source + self._pixel_width_source - cor_beta_x)) / (self._pixel_width_source**2) 
                    weight_upper_right = np.abs(self._source_min_y + n_y*self._pixel_width_source + self._pixel_width_source - cor_beta_y) * (
                        np.abs(self._source_min_x + n_x*self._pixel_width_source - cor_beta_x)) / (self._pixel_width_source**2)
                    weight_lower_left = np.abs(self._source_min_x + n_x*self._pixel_width_source + self._pixel_width_source - cor_beta_x) * (
                        np.abs(self._source_min_y + n_y*self._pixel_width_source - cor_beta_y)) / (self._pixel_width_source**2)
                    weight_lower_right = np.abs(self._source_min_x + n_x*self._pixel_width_source - cor_beta_x) * (
                        np.abs(self._source_min_y + n_y*self._pixel_width_source - cor_beta_y)) / (self._pixel_width_source**2)
...
=======
                    ...
                    weight_upper_right = (
                        ...
                        * (np.abs(self._minx + n_x * self._deltaPix - cor_beta_x))
                        / (self._deltaPix**2)
                    )
                    weight_lower_left = (
                        np.abs(
                        ...
                        * (np.abs(self._miny + n_y * self._deltaPix - cor_beta_y))
                        / (self._deltaPix**2)
                    )
                    weight_lower_right = (
                        np.abs(self._minx + n_x * self._deltaPix - cor_beta_x)
                    ...
                    if n_y + 1 < ny:
                        ...
                        if n_x + 1 < nx:
                            lensed_image[i, j] += (
                                image[n_y + 1, n_x + 1] * weight_lower_right
                            )

        return lensed_image

    def sparse_to_array(self, sparse):
        """Converts a sparse image representation (list of `[row_idx, col_idx, value]`
...
=======
        :param sparse: Sparse matrix representation (list of `[row_idx, col_idx, value]`
            tuples).
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
...
=======
    ...
    def sparse_convolve_and_dot_product(self, sp1, sp2, kernel):
        ...
        :param sp1: First sparse matrix representation.
        :param sp2: Second sparse matrix representation.
        :param kernel: The 2D PSF kernel (assumed odd dimensions, center at central
            pixel).
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
...
<<<<<<< HEAD
                ...
                if (kernel_center + delta_y >= 0 and kernel_center + delta_y < self._shape_kernel[0] and
                    kernel_center + delta_x >= 0 and kernel_center + delta_x < self._shape_kernel[1]):
                    inner_product += sp1[i][2] * sp2[j][2] * kernel[kernel_center + delta_y, kernel_center + delta_x]
...
=======
                ...
                inner_product += (
                    sp1[i][2]
                    * sp2[j][2]
                    * kernel[kernel_center + delta_x, kernel_center + delta_y]
                )
        return inner_product
...
<<<<<<< HEAD
            ...
            slice_y_end = np.min([kernel_center - y_sp + self._numPix, self._shape_kernel[0]])
            slice_x_start = np.max([kernel_center - x_sp, 0])
            slice_x_end = np.min([kernel_center - x_sp + self._numPix, self._shape_kernel[1]])
            
            convolved_image_y_start = y_sp - np.min([y_sp, kernel_center])
            convolved_image_y_end = y_sp + np.min([self._numPix - y_sp, self._shape_kernel[0] - kernel_center])
            convolved_image_x_start = x_sp - np.min([x_sp, kernel_center])
            convolved_image_x_end = x_sp + np.min([self._numPix - x_sp, self._shape_kernel[1] - kernel_center])
            
            convolved[convolved_image_y_start:convolved_image_y_end, convolved_image_x_start:convolved_image_x_end] += (
...
=======
            ...
            convolved += (
                val_sp * kernel[slice_x_start:slice_x_end, slice_y_start:slice_y_end]
            )
        return convolved
>>>>>>> 8389a9906b9db3877f2bc0296b72e90037f4489e
